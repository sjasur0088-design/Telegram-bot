...
TXT["ru"].update({
    ...
    "physical_q1": "Правила для въезжающих в Узбекистан",
    "physical_q2": "Правила для выезжающих из Узбекистана",
    "physical_q3": "Сколько телефонов можно привезти для себя",
    "physical_q4": "Сколько можно ввозить без пошлины",
    "physical_q5": "Правила по валюте для физлиц",
    "physical_q6": "Что будет при превышении лимита",
})
TXT["uz"].update({
    ...
    "physical_q1": "O‘zbekistonga kirayotganlar qoidalari",
    "physical_q2": "O‘zbekistondan chiqayotganlar qoidalari",
    "physical_q3": "O‘zingiz uchun nechta telefon olib kirish mumkin",
    "physical_q4": "Bojsiz qancha olib kirish mumkin",
    "physical_q5": "Jismoniy shaxslar uchun valyuta qoidalari",
    "physical_q6": "Limit oshsa nima bo‘ladi",
})
...
PHYSICAL_FAQ_FILE = os.path.join(BASE_DIR, "physical_faq.json")

def load_physical_faq_db() -> Dict[str, List[Dict[str, Any]]]:
    default_db = {"ru": [], "uz": []}
    try:
        if not os.path.exists(PHYSICAL_FAQ_FILE):
            return default_db
        with open(PHYSICAL_FAQ_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, dict):
            return {
                "ru": data.get("ru", []) if isinstance(data.get("ru", []), list) else [],
                "uz": data.get("uz", []) if isinstance(data.get("uz", []), list) else [],
            }
    except Exception as e:
        print(f"Failed to load physical_faq.json: {e}")
    return default_db

PHYSICAL_FAQ_DB = load_physical_faq_db()

def physical_entries(lang: str) -> List[Dict[str, Any]]:
    entries = PHYSICAL_FAQ_DB.get(lang)
    if isinstance(entries, list) and entries:
        return entries
    return PHYSICAL_FAQ_DB.get("ru", [])

def format_physical_entry(entry: Dict[str, Any]) -> str:
    answer = str(entry.get("answer", "")).strip()
    follow_up = str(entry.get("follow_up", "")).strip()
    if answer and follow_up:
        return answer + "\n\n" + follow_up
    return answer or follow_up

def build_physical_ready_map(lang: str) -> Dict[str, str]:
    entries = physical_entries(lang)
    labels = [
        t(lang, "physical_q1"),
        t(lang, "physical_q2"),
        t(lang, "physical_q3"),
    ...
    for i, label in enumerate(labels):
        if i < len(entries):
            result[label] = format_physical_entry(entries[i])
    return result

def search_physical_faq(query: str, lang: str) -> Dict[str, Any]:
    q = normalize_text(query)
    if not q:
        return {}
    best_entry = {}
    best_score = 0
    for entry in physical_entries(lang):
        score = 0
        ...
        if score > best_score:
            best_score = score
            best_entry = entry
    return best_entry if best_score >= 12 else {}

def looks_like_physical_commercial(text: str) -> bool:
    q = normalize_text(text)
    commercial_words = [
        "контейнер", "партия", "на продаж", "для продажи", "магазин", "оптом", "опт",
        "бизнес", "коммер", "юр лиц", "юрид", "фирм", "компания", "сертификат соответствия",
        "контракт", "инвойс на фирму", "декларант", "гтд", "broker", "brocker"
    ]
    personal_words = [
        "для себя", "личн", "подарок", "в ручной клади", "в багаже", "семь", "родствен",
        "jismoniy", "o'zim uchun", "ozim uchun", "shaxsiy"
    ]
    has_commercial = any(w in q for w in commercial_words)
    has_personal = any(w in q for w in personal_words)
    return has_commercial and not has_personal

def physical_scope_warning(lang: str) -> str:
    if lang == "ru":
        return ("Этот раздел только для физических лиц и личного пользования.\n\n"
                "Если товар ввозится для продажи, магазина, фирмы или другой коммерческой цели, "
                "нужно использовать раздел для юрлиц или специалиста.")
    return ("Bu bo‘lim faqat jismoniy shaxslar va shaxsiy foydalanish uchun.\n\n"
            "Agar tovar savdo, do‘kon, firma yoki boshqa tijorat maqsadi uchun olib kirilsa, "
            "yuridik shaxslar bo‘limi yoki mutaxassisga murojaat qiling.")
...
def physical_answer(q: str, lang: str) -> str:
    entry = search_physical_faq(q, lang)
    if entry:
        return format_physical_entry(entry)
    q_norm = normalize_text(q)
    if looks_like_physical_commercial(q_norm):
        return physical_scope_warning(lang)
    if any(x in q_norm for x in ["телефон", "iphone", "айфон", "смартфон", "phone"]):
        if lang == "ru":
            return (
                "Если вы везёте телефон для себя, обычно смотрят на 3 вещи: стоимость, количество и личное пользование.\n\n"
                "Напишите цену телефона, сколько штук и через что вы въезжаете — я объясню, будет ли пошлина."
            )
        return (
            "Telefonni o‘zingiz uchun olib kirayotgan bo‘lsangiz, odatda 3 narsa muhim: narxi, soni va shaxsiy foydalanish.\n\n"
            "Telefon narxini, nechta ekanini va qaysi yo‘l bilan kirayotganingizni yozing — boj bor-yo‘qligini tushuntiraman."
        )
    if any(x in q_norm for x in ["валют", "налич", "dollar", "dollor", "usd", "pul", "valyuta"]):
        if lang == "ru":
            return "По валюте для физлиц действуют отдельные правила. Напишите, пожалуйста, сколько денег вы ввозите или вывозите, и я объясню простыми словами."
        return "Valyuta bo‘yicha qoidalar jismoniy shaxslar uchun alohida qoida asosida qo‘llanadi. Qancha pul olib kirayotganingiz yoki olib chiqayotganingizni yozing."
    if any(x in q_norm for x in ["лекар", "dori", "medicina", "tablet"]):
        if lang == "ru":
            return (
                "Лекарства для личного пользования обычно можно ввозить, но по отдельным препаратам могут понадобиться подтверждающие документы.\n\n"
                "Напишите название лекарства и количество."
            )
        return (
            "Shaxsiy foydalanish uchun dorilarni odatda olib kirish mumkin, lekin ayrim dori vositalari uchun qo‘shimcha hujjat kerak bo‘lishi mumkin.\n\n"
            "Dori nomi va miqdorini yozing."
        )
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ready_answer = build_physical_ready_map(lang).get(text)
        if ready_answer:
    ...
    if c["mode"] == "physical_chat":
        ...
        base_answer = physical_answer(text, lang)

        if looks_like_physical_commercial(text):
            await message.answer(base_answer + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
            return

        local_entry = search_physical_faq(text, lang)
        if local_entry:
            await message.answer(format_physical_entry(local_entry) + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
            return

        if client:
            try:
                prompt = (
                    f"Ответь простыми словами на {'русском' if lang=='ru' else 'узбекском'} языке как AI таможенник для физических лиц в Узбекистане. "
                    "Ты отвечаешь только для личного пользования и некоммерческого ввоза/вывоза. "
                    "Нельзя путать физлиц и юрлиц. Не уходи в профессиональную лексику. "
                    "Сначала дай короткий понятный ответ, потом 2-4 коротких пункта, потом задай один уточняющий вопрос. "
                    f"Локальная база правил: {json.dumps(physical_entries(lang), ensure_ascii=False)} "
                    f"Запрос пользователя: {text}"
                )
                resp = client.responses.create(model=OPENAI_MODEL, input=prompt)
                ans = (resp.output_text or "").strip()
                await message.answer((ans or base_answer) + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
            except Exception:
                await message.answer(base_answer + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
        else:
            await message.answer(base_answer + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
