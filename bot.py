...
TXT = {
    "ru": {
        ...
        "physical_intro":"🤖 AI таможенник\n\nВыберите:\n• готовые вопросы\n• или задайте свой вопрос по таможенным правилам для физических лиц в Узбекистане.\n\nЕсли точного ответа не хватит, я предложу помощь специалиста.",
        ...
        "physical_no_calc":"Для физлиц доступны только: AI таможенник, список таможенных постов и специалист.",
        ...
        "faq_intro":"Выберите готовый вопрос или задайте свой вопрос через AI таможенник.",
        "docs_physical":"Выберите тип постов: приграничные или ВЭД посты.",
    ...
    "uz": {
        ...
        "chat":"🤖 AI bojxonachi",
        "faq":"📋 Tayyor savollar",
        "docs":"🏢 Bojxona postlari ro‘yxati",
        ...
        "physical_intro":"🤖 AI bojxonachi\n\nTanlang:\n• tayyor savollar\n• yoki o‘z savolingizni yozing.\n\nAgar aniq javob yetarli bo‘lmasa, mutaxassis yordamni taklif qilaman.",
        ...
        "physical_no_calc":"Jismoniy shaxslar uchun faqat: AI bojxonachi, bojxona postlari ro‘yxati va mutaxassis mavjud.",
        ...
        "faq_intro":"Tayyor savolni tanlang yoki AI bojxonachi orqali o‘z savolingizni yozing.",
        "docs_physical":"Post turini tanlang: chegara postlari yoki VED postlari.",
...
def physical_posts_kb(lang: str):
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    if lang == "ru":
        kb.add("🚧 Приграничные посты")
        kb.add("📦 ВЭД посты")
    else:
        kb.add("🚧 Chegara postlari")
        kb.add("📦 VED postlari")
    kb.add(t(lang, "back"), t(lang, "back_menu"))
    return kb
...
async def router(message: types.Message):
    ...
    if role == "physical":
        if text == t(lang, "chat"):
            ...
            await message.answer(t(lang, "physical_intro"), reply_markup=physical_ai_kb(lang)); return
        if text == t(lang, "faq"):
            reset_mode(uid); c["mode"] = "physical_ready_questions"
            await message.answer("Выберите готовый вопрос:" if lang == "ru" else "Tayyor savolni tanlang:", reply_markup=physical_ready_kb(lang)); return
        if text == t(lang, "ask_own"):
            reset_mode(uid); c["mode"] = "physical_chat"
            await message.answer("Напишите ваш вопрос по таможенным правилам для физических лиц в Узбекистане." if lang == "ru" else "Jismoniy shaxslar uchun bojxona qoidalari bo‘yicha savolingizni yozing.", reply_markup=physical_ai_kb(lang)); return
        if text == t(lang, "docs"):
            reset_mode(uid); c["mode"] = "physical_posts_menu"
            await message.answer(TXT[lang]["docs_physical"], reply_markup=physical_posts_kb(lang)); return
        if text in ["🚧 Приграничные посты", "🚧 Chegara postlari"]:
            title = "Приграничные посты Республики Узбекистан" if lang == "ru" else "O‘zbekiston Respublikasi chegara postlari"
            await send_safe_message(message, format_posts_list(PHYSICAL_BORDER_POSTS, lang, title), reply_markup=physical_posts_kb(lang)); return
        if text in ["📦 ВЭД посты", "📦 VED postlari"]:
            title = "ВЭД посты Республики Узбекистан" if lang == "ru" else "O‘zbekiston Respublikasi VED postlari"
            await send_safe_message(message, format_posts_list(PHYSICAL_VED_POSTS, lang, title), reply_markup=physical_posts_kb(lang)); return
    ...
    if c["mode"] == "physical_ai_menu":
        await message.answer(t(lang, "physical_intro"), reply_markup=physical_ai_kb(lang))
        return

    if c["mode"] == "physical_ready_questions":
        qa = PHYSICAL_READY_QA[lang]
        if text in qa:
            await message.answer(qa[text] + "\n\n" + t(lang, "free_specialist"), reply_markup=physical_ready_kb(lang))
            return
        await message.answer("Выберите готовый вопрос." if lang == "ru" else "Tayyor savolni tanlang.", reply_markup=physical_ready_kb(lang))
        return

    if c["mode"] == "physical_posts_menu":
        await message.answer(TXT[lang]["docs_physical"], reply_markup=physical_posts_kb(lang))
        return

    if c["mode"] == "physical_chat":
        if not looks_like_customs_question(text):
            await message.answer(t(lang, "only_customs"), reply_markup=physical_ai_kb(lang))
            return
        direct = None
        if lang == "ru":
            qa_map = PHYSICAL_READY_QA["ru"]
        else:
            qa_map = PHYSICAL_READY_QA["uz"]
        for qk, qv in qa_map.items():
            if normalize_text(qk) in normalize_text(text):
                direct = qv
                break
        if direct:
            await message.answer(direct + "\n\n" + t(lang, "free_specialist"), reply_markup=physical_ai_kb(lang))
            return
        if client:
            try:
                rules_ru = "Для физлиц действуют нормы беспошлинного ввоза: аэропорт до 2000 USD, ж/д и речные пункты до 1000 USD, автодорожные и пешеходные пункты до 300 USD. Алкоголь до 2 литров, табачные изделия до 10 пачек, духи до 3 единиц, ювелирные изделия до 65 граммов. При превышении нормы единый таможенный платеж 30% от таможенной стоимости, но не менее 3 USD/кг. Телефонный аппарат обычно 1 единица в 6 календарных месяцев через автодорожные, пешеходные, ж/д и речные пункты."
                rules_uz = "Jismoniy shaxslar uchun bojsiz olib kirish me’yori: aeroport orqali 2000 USDgacha, temir yo‘l va daryo punktlari orqali 1000 USDgacha, avtoyo‘l va piyoda punktlari orqali 300 USDgacha. Alkogol 2 litrgacha, tamaki 10 pachkagacha, atir 3 donagacha, zargarlik buyumlari 65 grammgacha. Norma oshsa yagona bojxona to‘lovi 30%, lekin kamida 3 USD/kg. Telefon apparati odatda 6 oyda 1 dona."
                rules = rules_ru if lang == "ru" else rules_uz
                prompt = (f"Ответь кратко и понятно на {'русском' if lang=='ru' else 'узбекском'} языке как AI таможенник по вопросам физических лиц в Узбекистане. "
                          f"Отвечай только по теме таможни. Сначала опирайся на эти правила: {rules}. "
                          f"Если точного ответа нет, так и скажи и предложи обратиться к специалисту. Запрос: {text}")
                resp = client.responses.create(model=OPENAI_MODEL, input=prompt)
                ans = (resp.output_text or "").strip()
                await message.answer((ans or physical_answer(text, lang)) + "\n\n" + t(lang, "free_specialist"), reply_markup=physical_ai_kb(lang))
            except Exception:
                await message.answer(physical_answer(text, lang) + "\n\n" + t(lang, "free_specialist"), reply_markup=physical_ai_kb(lang))
        else:
            await message.answer(physical_answer(text, lang) + "\n\n" + t(lang, "free_specialist"), reply_markup=physical_ai_kb(lang))
        return
