...
def format_posts_list(posts, lang):
    ...
    parts = [f"<b>{title}</b>",""]
    for p in posts:
        parts.append(f"<b>{p['num']}. {p['name']}</b>")
        if p.get("location"):
            parts.append(("Локация: " if lang == "ru" else "Joylashuv: ") + p["location"])
        if p.get("phone"):
            parts.append(("Контакт: " if lang == "ru" else "Kontakt: ") + p["phone"])
        if p.get("code"):
            parts.append(("Код поста: " if lang == "ru" else "Post kodi: ") + str(p["code"]))
        parts.append("")
    return "\n".join(parts).strip()
