...
async def router(message: types.Message):
    ...
    if role == "broker":
        if text == t(lang, "broker_cost"):
            reset_mode(uid); c["form_data"] = {"service":"Анализ таможенной стоимости"}; c["pending_form"] = "broker_name"
            await message.answer(t(lang, "broker_paid") + "\n\n" + t(lang, "enter_name"), reply_markup=broker_menu(lang))
            return
        if text == t(lang, "broker_docs"):
            reset_mode(uid); c["form_data"] = {"service":"Проверка документов перед подачей"}; c["pending_form"] = "broker_name"
            await message.answer(t(lang, "broker_paid") + "\n\n" + t(lang, "enter_name"), reply_markup=broker_menu(lang))
            return
        if text == t(lang, "broker_cert"):
            reset_mode(uid); c["form_data"] = {"service":"Нюансы по сертификации"}; c["pending_form"] = "broker_name"
            await message.answer(t(lang, "broker_paid") + "\n\n" + t(lang, "enter_name"), reply_markup=broker_menu(lang))
            return
        if text == t(lang, "broker_tnved_analytics"):
            reset_mode(uid); c["form_data"] = {"service":"Аналитика по ТН ВЭД коду"}; c["pending_form"] = "broker_name"
            await message.answer(t(lang, "broker_paid") + "\n\n" + t(lang, "enter_name"), reply_markup=broker_menu(lang))
            return
