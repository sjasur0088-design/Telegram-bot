...
TXT["ru"].update({
    ...
    "physical_only_customs": "Я помогаю по таможенным вопросам Узбекистана для личного пользования. Напишите, что именно вы хотите ввезти, вывезти или уточнить.",
...
TXT["uz"].update({
    ...
    "physical_ask_own_intro": "Shaxsiy foydalanish uchun bojxona bo‘yicha savolingizni yozing. Masalan: Dubaydan iPhone olib kelsam nima bo‘ladi, nechta telefon olib kirish mumkin, dori olib kirish mumkinmi.",
    "physical_free_specialist": "Xohlasangiz, mutaxassis sizga kun davomida bepul javob beradi.",
    "physical_only_customs": "Men O‘zbekiston bojxona qoidalari bo‘yicha, ayniqsa shaxsiy foydalanish uchun savollarda yordam beraman. Nima olib kirayotganingiz yoki nimani aniqlashtirmoqchi ekaningizni yozing.",
...
def looks_like_physical_customs_question(text: str) -> bool:
    ...
    if not q:
        return False
    keywords = [
        "bojxona", "тамож", "таможн", "ввоз", "вывоз", "olib kel", "olib kir", "olib chiq",
        "iphone", "telefon", "смартфон", "телефон", "imei", "dubay", "dubai",
        "dori", "dorilar", "лекар", "таблет", "bagaj", "багаж", "aeroport", "аэропорт",
        "valyuta", "валют", "usd", "dollar", "limit", "meyor", "норма", "bojsiz", "без пошлины",
        "posilka", "pochta", "почта", "kuryer", "курьер", "shaxsiy", "личн", "для себя",
        "sovg", "подар", "noutbuk", "ноутбук", "planshet", "планшет", "televizor", "телевизор",
        "kir yuvish", "стираль", "post", "пост", "chegara"
    ]
    return any(k in q for k in keywords) or looks_like_customs_question(text)

def is_obviously_not_customs(text: str) -> bool:
    q = normalize_text(text)
    bad = ["футбол", "матч", "кино", "музыка", "анекдот", "погода", "sport", "game"]
    return any(x in q for x in bad)
...
def physical_answer(q: str, lang: str) -> str:
    ...
    if any(x in qn for x in ["iphone", "телефон", "смартфон", "telefon", "imei"]):
        return (
            "Agar telefon yoki iPhone olib kelsangiz, asosan 4 narsa muhim: bu shaxsiy foydalanish uchunmi, nechta dona olib kelyapsiz, yangi yoki ishlatilganmi va IMEI ro‘yxatdan o‘tkazish kerak bo‘ladimi. Agar 1 dona shaxsiy foydalanish uchun bo‘lsa, odatda masala yengilroq bo‘ladi. Agar bir nechta bo‘lsa, bojxona buni tijorat olib kirish deb baholashi mumkin. Modeli va sonini yozsangiz, aniqroq tushuntiraman."
        ) if lang == "uz" else (
            "Если вы ввозите телефон или iPhone, обычно важны 4 вещи: это для личного пользования или нет, сколько устройств вы везёте, новые они или б/у, и потребуется ли регистрация IMEI. Если это 1 устройство для себя, обычно ситуация проще. Если устройств несколько, таможня может расценить это как коммерческий ввоз. Напишите модель и количество — я объясню точнее."
        )

    if any(x in qn for x in ["dori", "dorilar", "лекар", "таблет"]):
        return (
            "Dorilarni olib kirishda tarkibi, miqdori, nima maqsadda olib kirilayotgani va ayrim moddalarga cheklovlar bor-yo‘qligi muhim. Agar bu shaxsiy foydalanish uchun bo‘lsa, odatda yondashuv yengilroq bo‘ladi, lekin ayrim dorilar bo‘yicha qo‘shimcha tekshiruv kerak bo‘lishi mumkin. Dori nomi va miqdorini yozsangiz, aniqroq aytaman."
        ) if lang == "uz" else (
            "При ввозе лекарств важны состав, количество, цель ввоза и наличие ограничений по отдельным веществам. Если это для личного пользования, подход обычно мягче, но по некоторым препаратам может понадобиться дополнительная проверка. Напишите название лекарства и количество — я отвечу точнее."
        )

    if any(x in qn for x in ["valyuta", "валют", "usd", "dollar"]):
        return (
            "Jismoniy shaxslar O‘zbekistondan naqd valyutani 100 million so‘m ekvivalentidan oshmagan summada olib chiqishlari mumkin. Agar sizda kirish yoki chiqish holati bo‘yicha aniq savol bo‘lsa, vaziyatni yozing."
        ) if lang == "uz" else (
            "Физические лица могут вывозить наличную валюту из Узбекистана в сумме, не превышающей эквивалент 100 миллионов сумов. Если у вас конкретная ситуация по ввозу или вывозу валюты, опишите её подробнее."
        )

    if any(x in qn for x in ["aeroport", "аэропорт", "без пошлины", "bojsiz", "норма", "meyor", "limit"]):
        return (
            "Jismoniy shaxslar uchun bojsiz olib kirish me’yorlari kirish turiga qarab farq qiladi: aeroport orqali 2000 USDgacha, temir yo‘l va daryo punktlari orqali 1000 USDgacha, avtoyo‘l va piyoda punktlari orqali 300 USDgacha. Agar aynan nima olib kirayotganingizni yozsangiz, qaysi me’yor qo‘llanishini tushuntiraman."
        ) if lang == "uz" else (
            "Для физлиц нормы беспошлинного ввоза зависят от типа въезда: через аэропорт — до 2000 USD, через железнодорожные и речные пункты — до 1000 USD, через автодорожные и пешеходные пункты — до 300 USD. Если напишете, что именно вы ввозите, я поясню, какая норма применяется."
        )

    return (
        "Savolingiz bojxona va shaxsiy foydalanish bilan bog‘liq bo‘lsa, mahsulot yoki vaziyatni biroz aniqroq yozing — men tushuntirib beraman."
    ) if lang == "uz" else (
        "Если вопрос связан с таможней и личным пользованием, напишите товар или ситуацию чуть точнее — я объясню подробнее."
    )
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_post_card":
        if text == t(lang, "physical_location"):
            ...
            if not post or post.get("lat") is None or post.get("lon") is None:
                await message.answer(t(lang, "physical_no_location"), reply_markup=physical_post_card_kb(lang, False))
                return
    ...
    if c["mode"] == "physical_chat":
        if is_obviously_not_customs(text):
            await message.answer(t(lang, "physical_only_customs"), reply_markup=physical_ai_kb(lang))
            return

        if not looks_like_physical_customs_question(text):
            await message.answer(t(lang, "physical_only_customs"), reply_markup=physical_ai_kb(lang))
            return

        local_answer = physical_answer(text, lang)

        if client:
            try:
                faq_values = list(PHYSICAL_FAQ.get(lang, {}).values()) if isinstance(PHYSICAL_FAQ.get(lang, {}), dict) else []
                prompt = (
                    ("Javobni o‘zbek tilida bering. " if lang == "uz" else "Ответь на русском языке. ")
                    + ("Siz O‘zbekiston bojxona qoidalari bo‘yicha jismoniy shaxslar uchun AI yordamchisiz. " if lang == "uz" else "Ты AI-помощник по таможенным правилам Узбекистана для физических лиц. ")
                    + ("Bu bo‘lim faqat shaxsiy foydalanish uchun. Yuridik shaxslar, brokerlik yoki tijorat importiga kirib ketmang. " if lang == "uz" else "Этот раздел только для личного пользования. Не уходи в тему юрлиц, брокеров и коммерческого импорта. ")
                    + ("Avval shu tayyor qoidalar va mahalliy javoblarni hisobga oling: " if lang == "uz" else "Сначала учитывай эти готовые правила и локальные ответы: ")
                    + json.dumps(faq_values[:12], ensure_ascii=False)
                    + (". Agar bu ma’lumotlar yetarli bo‘lmasa, oddiy aqlli yordamchi kabi mantiqan tushuntir. Juda erta rad etma. " if lang == "uz" else ". Если этих данных недостаточно, объясни ситуацию как обычный умный помощник. Не отказывай слишком рано. ")
                    + ("Agar savol telefon, iPhone, dori, valyuta, aeroport, limit, bojsiz me’yor, IMEI, posilka yoki bojxona postlari haqida bo‘lsa, aniq va sodda javob ber. " if lang == "uz" else "Если вопрос про телефон, iPhone, лекарства, валюту, аэропорт, лимиты, беспошлинный ввоз, IMEI, посылки или таможенные посты — ответь понятно и по существу. ")
                    + ("Kerak bo‘lsa bitta aniqlashtiruvchi savol bering. Javobni tabiiy usulda yozing, 1-2 kichik xatboshida. Oxirida mutaxassis yordamni yumshoq taklif qiling. Savol: " if lang == "uz" else "При необходимости задай один уточняющий вопрос. Пиши естественно, 1-2 небольшими абзацами. В конце мягко предложи помощь специалиста. Вопрос: ")
                    + text
                )
                resp = client.responses.create(model=OPENAI_MODEL, input=prompt)
                ans = (resp.output_text or "").strip()
                final_answer = ans or local_answer
            except Exception:
