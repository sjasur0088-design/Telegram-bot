...
PHYSICAL_FAQ = {
    "ru": {
        "Таможенные правила для физлиц": """Для физических лиц при въезде в Узбекистан действуют нормы беспошлинного ввоза. Через аэропорты лимиты обычно выше, чем через автодорожные пункты. Также есть отдельные ограничения по алкоголю, табачным изделиям, парфюмерии и некоторым другим товарам.

Если хотите, напишите, через какой пункт пропуска вы въезжаете, и я объясню точнее.""",
        "Сколько можно ввозить без пошлины": """Это зависит от способа въезда: аэропорт, поезд, авто или пеший переход. Для личного пользования действуют разные лимиты.

Напишите, откуда и как именно въезжаете, и я отвечу точнее.""",
        "Сколько телефонов можно привезти": """Если телефон ввозится для личного пользования, обычно важны количество, частота ввоза и признаки коммерческой партии. Если это 1 телефон для себя, ситуация обычно проще. Если телефонов несколько, таможня может расценить это как не для личного пользования.

Напишите, сколько телефонов и какие модели вы везёте.""",
        "Можно ли ввозить лекарства": """Да, для личного пользования лекарства ввозить можно, но важны состав, количество и назначение. По отдельным препаратам могут быть ограничения.

Напишите название лекарства и количество, я объясню точнее.""",
        "Сколько можно вывозить валюты": """Для вывоза наличной валюты действуют установленные правила и лимиты. Для точного ответа важно понимать сумму и вид валюты.

Напишите, сколько и какой валюты вы хотите вывезти.""",
        "Что будет при превышении нормы": """Если норма беспошлинного ввоза превышена, могут применяться таможенные платежи. Точный расчёт зависит от товара, стоимости, веса и способа ввоза.

Напишите, какой именно товар и на какую сумму вы ввозите.""",
    },
    "uz": {
        "Jismoniy shaxslar uchun bojxona qoidalari": """Jismoniy shaxslar uchun O‘zbekistonga kirishda bojsiz olib kirish me’yorlari mavjud. Aeroport, avtoyo‘l, temir yo‘l va boshqa yo‘nalishlarda me’yorlar farq qilishi mumkin. Shuningdek, alkogol, tamaki, atir-upa va ayrim tovarlar bo‘yicha alohida cheklovlar bor.

Qaysi post yoki qaysi yo‘l orqali kirayotganingizni yozsangiz, aniqroq tushuntiraman.""",
        "Bojsiz qancha olib kirish mumkin": """Bu kirish usuliga bog‘liq: aeroport, poyezd, avtoyo‘l yoki piyoda o‘tish. Shaxsiy foydalanish uchun turli limitlar qo‘llanadi.

Qayerdan va qaysi yo‘l bilan kirayotganingizni yozing, aniqroq javob beraman.""",
        "Nechta telefon olib kirish mumkin": """Telefonni shaxsiy foydalanish uchun olib kirishda soni, tez-tez olib kirish holati va tijorat alomatlari muhim. Agar 1 dona telefon o‘zingiz uchun bo‘lsa, odatda masala osonroq bo‘ladi. Agar bir nechta telefon bo‘lsa, bojxona buni tijorat olib kirish deb baholashi mumkin.

Nechta telefon va qaysi modellar ekanini yozing.""",
        "Dori olib kirish mumkinmi": """Ha, shaxsiy foydalanish uchun dori olib kirish mumkin, lekin tarkibi, miqdori va maqsadi muhim. Ayrim dori vositalari uchun qo‘shimcha cheklovlar yoki hujjatlar kerak bo‘lishi mumkin.

Dori nomi va miqdorini yozing, aniqroq tushuntiraman.""",
        "Qancha valyuta olib chiqish mumkin": """Naqd valyutani olib chiqish bo‘yicha belgilangan qoidalar va limitlar bor. Aniq javob uchun summa va valyuta turini bilish kerak.

Qancha va qaysi valyutani olib chiqmoqchisiz?""",
        "Norma oshsa nima bo‘ladi": """Agar bojsiz me’yor oshsa, bojxona to‘lovlari qo‘llanishi mumkin. Aniq hisob-kitob tovar turi, qiymati, vazni va olib kirish usuliga bog‘liq.
    ...
    if c["mode"] == "physical_chat":
        ...
        if ready_answer:
            await send_safe_message(
                message,
                ready_answer + "\n\n" + t(lang, "free_specialist"),
                reply_markup=physical_menu(lang)
            )
            return
        ...
        if any(x in user_q for x in commercial_markers):
            answer = (
                "Я помогаю физическим лицам только по вопросам личного пользования. "
                "Если вопрос связан с коммерческим импортом, лучше перейти в раздел для юр лиц или обратиться к специалисту."
                if lang == "ru"
                else
                "Men jismoniy shaxslar uchun faqat shaxsiy foydalanish bo‘yicha yordam beraman. "
                "Agar savol tijorat importiga oid bo‘lsa, yuridik shaxslar bo‘limiga o‘ting yoki mutaxassisga murojaat qiling."
            )
            await message.answer(answer, reply_markup=physical_menu(lang))
            return

        customs_markers = [
            "bojxona", "olib kir", "olib chiq", "telefon", "iphone", "dori", "valyuta",
            "limit", "aeroport", "imei", "posilka", "post", "тамож", "ввоз", "вывоз",
            "телефон", "айфон", "лекар", "валют", "лимит", "аэропорт", "име", "посылк", "пост",
            "dubay", "дубай", "bagaj", "багаж"
        ]

        if not any(x in user_q for x in customs_markers):
            answer = (
                "Я помогаю только по таможенным вопросам для физических лиц в Узбекистане. "
                "Например: телефоны, лекарства, лимиты, валюта, аэропорт, IMEI, личный ввоз."
                if lang == "ru"
                else
                "Men O‘zbekistonda jismoniy shaxslar uchun faqat bojxona savollariga yordam beraman. "
                "Masalan: telefon, dori, limit, valyuta, aeroport, IMEI, shaxsiy olib kirish."
            )
            await message.answer(answer, reply_markup=physical_menu(lang))
            return
        ...
        if client:
            try:
                system_prompt = (
                    ...
                    "Не отказывай слишком рано. Если данных не хватает, задай 1 уточняющий вопрос. "
                    ...
                    "Juda tez rad javobini bermagin. Agar ma’lumot yetmasa, 1 ta qisqa aniqlashtiruvchi savol ber. "
                ...
                await send_safe_message(message, final_answer, reply_markup=physical_menu(lang))
                return
            except Exception:
                pass
