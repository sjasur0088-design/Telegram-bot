...
TXT["ru"].update({
    ...
    "physical_no_location": "Для этого поста геопозиция пока не указана.",
...
TXT["uz"].update({
    ...
    "physical_no_location": "Bu post uchun geolokatsiya hali kiritilmagan.",
...
def _safe_post_coords(post: Dict[str, Any]) -> tuple[Optional[float], Optional[float]]:
    if not post:
        return None, None
    lat = post.get("lat")
    lon = post.get("lon")
...
def _post_has_location(post: Dict[str, Any]) -> bool:
    lat, lon = _safe_post_coords(post)
...
async def router(message: types.Message):
    ...
    if c["mode"] in ["physical_posts_border", "physical_posts_ved"]:
        ...
        has_loc = _post_has_location(post)
    ...
    if c["mode"] == "physical_post_card":
        if text == t(lang, "physical_location"):
            post = c["form_data"].get("selected_post")
            if not post:
                kind = c["form_data"].get("selected_post_kind")
                num = c["form_data"].get("selected_post_num")
                post = find_post_by_num(kind, str(num)) if kind and num is not None else None
            lat, lon = _safe_post_coords(post)
            ...
            return
        selected_post = c["form_data"].get("selected_post")
        await message.answer(t(lang, "physical_free_specialist"), reply_markup=physical_post_card_kb(lang, _post_has_location(selected_post)))
        return
    ...
    if c["mode"] == "physical_chat":
        ...
        if client:
            try:
                prompt = (
                    f"Ответь кратко на {'русском' if lang=='ru' else 'узбекском'} языке как помощник по таможенным вопросам Узбекистана для физических лиц. "
                    "Отвечай только по теме таможни, лимитов ввоза/вывоза, документов, лекарств, телефонов, IMEI, валюты и таможенных постов. "
                    "Не показывай брокерские ставки по умолчанию. "
                    f"Готовые правила: {json.dumps(list(PHYSICAL_FAQ.get(lang, {}).values()), ensure_ascii=False)} "
                    f"Запрос: {text}"
                )
                resp = client.responses.create(model=OPENAI_MODEL, input=prompt)
