...
from physical_router_clean import load_physical_router
from physical_document_engine_clean import load_physical_document_engine
from physical_context_clean import get_physical_context, save_physical_context, clear_physical_context
...
PHYSICAL_KNOWLEDGE_PATH = os.path.join(BASE_DIR, "physical_knowledge_pro_clean.json")
try:
    PHYSICAL_ROUTER = load_physical_router(PHYSICAL_KNOWLEDGE_PATH)
    PHYSICAL_ENGINE = load_physical_document_engine(PHYSICAL_KNOWLEDGE_PATH)
except Exception as _physical_boot_error:
    print("PHYSICAL PRO INIT ERROR:", _physical_boot_error)
    PHYSICAL_ROUTER = None
...
def normalize_button_text(v: str) -> str:
    s = (v or "").strip().lower()
    s = s.replace("ʻ", "'").replace("’", "'").replace("`", "'").replace("ʼ", "'").replace("‘", "'")
    s = s.replace("o‘z", "o'z").replace("bo‘", "bo'").replace("ro‘", "ro'")
    s = re.sub(r"^[^\w']+", "", s, flags=re.UNICODE)
    s = re.sub(r"\s+", " ", s, flags=re.UNICODE).strip()
    return s


def button_eq(user_text: str, expected_text: str) -> bool:
    return normalize_button_text(user_text) == normalize_button_text(expected_text)
...
def load_physical_faq_pro() -> Dict[str, Any]:
    candidates = [
        os.path.join(BASE_DIR, "physical_faq_document_first.json"),
        os.path.join(BASE_DIR, "physical_faq.json"),
    ]
    empty = {"ru": {"faq_items": [], "global_rules": {}}, "uz": {"faq_items": [], "global_rules": {}}}
    try:
        raw = None
        chosen_path = None
        ...
        prepared = {"ru": {"faq_items": [], "global_rules": {}}, "uz": {"faq_items": [], "global_rules": {}}}
        for lang in ("ru", "uz"):
            block = raw.get(lang, {}) if isinstance(raw, dict) else {}
            if not isinstance(block, dict):
                continue

            faq_items = block.get("faq_items", [])
            if isinstance(faq_items, list):
                prepared[lang]["faq_items"] = faq_items

            raw_rules = block.get("global_rules", {})
            normalized_rules = {}
            if isinstance(raw_rules, dict):
                # New compact format: direct string keys import/med/currency/auto
                for key in ("import", "med", "medicine", "currency", "auto"):
                    val = raw_rules.get(key)
                    if isinstance(val, str) and val.strip():
                        normalized_rules[key] = val.strip()

                # Backward compatibility with nested old format
                mapping = {
                    "import_limits": "import",
                    "single_customs_payment": "import",
                    "currency_rules": "currency",
                    "physical_only": "import",
                    "short_trip_rule": "import",
                    "commercial_redirect": "import",
                }
                for old_key, new_key in mapping.items():
                    val = raw_rules.get(old_key)
                    if isinstance(val, dict):
                        answer = str(val.get("answer", "")).strip()
                        if answer and new_key not in normalized_rules:
                            normalized_rules[new_key] = answer
                    elif isinstance(val, str) and val.strip() and new_key not in normalized_rules:
                        normalized_rules[new_key] = val.strip()

            prepared[lang]["global_rules"] = normalized_rules

        if chosen_path:
            print(f"Loaded physical FAQ source: {os.path.basename(chosen_path)}")
        return prepared
    except Exception as e:
        print(f"Failed to load physical FAQ source: {e}")
        return empty
...
def build_physical_ai_prompt(user_text: str, c: Dict[str, Any], lang: str) -> str:
    topic = physical_rule_topic(c) or physical_get_pro_topic(c) or physical_rule_topic_from_text(user_text) or physical_pro_guess_topic(user_text)
    base_answer = physical_rule_base_answer(user_text, lang, topic)
    if lang == "uz":
        base = (
            "Sen O‘zbekiston bojxona masalalari bo‘yicha jismoniy shaxslar uchun yordamchisan. "
            "Foydalanuvchiga avval berilgan bazaviy qoida ustiga aniqlashtiruvchi qisqa javob ber. "
            "MUHIM: hech qachon yangi limit, eski norma, foiz, dollar, euro yoki kilogramm bo‘yicha raqamni o‘zingdan qo‘shma. "
            "Faqat lokal kontekstda berilgan raqam va qoidalarga tayangan holda yoz. "
            "Agar lokal kontekstda aniq raqam bo‘lmasa, raqam aytma — 'aniq summa uchun mutaxassis tekshirib bersin' deb yoz. "
            "Javob 2-4 gapdan oshmasin. Yuridik shaxslar, broker yoki TN VED tomon ketma."
        )
    else:
        base = (
            "Ты помощник по таможенным вопросам Узбекистана для физических лиц. "
            "Дай короткий уточняющий ответ поверх уже выданного базового правила. "
            "КРИТИЧЕСКИ ВАЖНО: никогда не придумывай новые лимиты, старые нормы, проценты, доллары, евро или килограммы. "
            "Используй только цифры и правила из локального контекста ниже. "
            "Если в локальном контексте нет точной цифры, не называй её и не угадывай — напиши, что для точной суммы лучше уточнить у специалиста. "
            "Ответ максимум 2-4 предложения. Не уводи в юрлица, брокера или ТН ВЭД."
        )
    local = []
    if topic:
        local.append(f"Тема: {topic}" if lang == "ru" else f"Mavzu: {topic}")
    if base_answer:
        local.append(base_answer)
    source_text = (physical_rule_state(c).get("source_text") or physical_pro_state(c).get("source_text") or "").strip()
    if source_text:
        local.append(("Предыдущий вопрос: " if lang == "ru" else "Oldingi savol: ") + source_text)
    return base + ("\n\nЛокальный контекст:\n- " if lang == "ru" else "\n\nMahalliy kontekst:\n- ") + "\n- ".join([x for x in local if x])
...
def physical_rule_state(c: Dict[str, Any]) -> Dict[str, Any]:
    return c.setdefault("physical_rule_state", {})


def physical_clear_rule_state(c: Dict[str, Any]) -> None:
    c["physical_rule_state"] = {}


def physical_set_rule_topic(c: Dict[str, Any], topic: str, source_text: str = "") -> None:
    prev = physical_rule_state(c)
    c["physical_rule_state"] = {
        "topic": topic or "",
        "base_answer_sent": bool(topic),
        "source_text": (source_text or "")[:500],
        "currency_direction": prev.get("currency_direction", ""),
        "currency_amount": prev.get("currency_amount", ""),
    }


def physical_rule_topic(c: Dict[str, Any]) -> str:
    return physical_rule_state(c).get("topic", "")


def physical_extract_money_amount(text: str) -> str:
...
def physical_update_currency_state(c: Dict[str, Any], text: str, lang: str) -> Dict[str, Any]:
    st = physical_rule_state(c)
    q = normalize_text(text)
    direction = st.get("currency_direction", "")
    amount = st.get("currency_amount", "")

    export_words = ["вывоз", "вывожу", "вывезти", "вывозите", "olib chiq", "olib ket", "chiqish"]
    import_words = ["ввоз", "ввожу", "ввезти", "ввозите", "olib kir", "olib kel", "kirish"]

    if any(x in q for x in export_words):
        direction = "export"
    elif any(x in q for x in import_words):
        direction = "import"

    extracted = physical_extract_money_amount(text)
    if extracted:
        amount = extracted

    st["currency_direction"] = direction
    st["currency_amount"] = amount
    c["physical_rule_state"] = st
    return st


def physical_currency_followup_answer(c: Dict[str, Any], text: str, lang: str) -> str:
    st = physical_update_currency_state(c, text, lang)
    direction = st.get("currency_direction", "")
    amount = st.get("currency_amount", "")

    if not amount and not direction:
        return (
            "По валюте в этих справочных таблицах нет точного лимита по сумме. Напишите сумму и уточните: ввоз или вывоз."
            if lang == "ru" else
            "Valyuta bo‘yicha bu jadval va ilovalarda aniq summa limiti ko‘rsatilmagan. Summani va bu olib kirishmi yoki olib chiqishmi — yozing."
        )

    if amount and not direction:
        return (
            f"Сумма: {amount}. Теперь уточните: это ввоз или вывоз валюты?"
            if lang == "ru" else
            f"Summa: {amount}. Endi bu valyutani olib kirishmi yoki olib chiqishmi — aniqlashtiring."
        )

    if direction and not amount:
        return (
            "Уточните сумму валюты."
            if lang == "ru" else
            "Valyuta summasini aniqlashtiring."
        )

    if direction == "export":
        return (
            f"Вывоз валюты: сумма {amount}. По валюте действует отдельный порядок декларирования. В этих двух справочниках точный лимит по сумме не указан, поэтому для точного ответа лучше проверить отдельные валютные правила или обратиться к специалисту."
            if lang == "ru" else
            f"Valyutani olib chiqish: summa {amount}. Valyuta bo‘yicha alohida deklaratsiya tartibi qo‘llanadi. Ushbu ikki справочникda aniq summa limiti ko‘rsatilmagan, shu sabab aniq javob uchun alohida valyuta qoidalarini tekshirish yoki mutaxassisga murojaat qilish kerak."
        )

    return (
        f"Ввоз валюты: сумма {amount}. По валюте действует отдельный порядок декларирования. В этих двух справочниках точный лимит по сумме не указан, поэтому для точного ответа лучше проверить отдельные валютные правила или обратиться к специалисту."
        if lang == "ru" else
        f"Valyutani olib kirish: summa {amount}. Valyuta bo‘yicha alohida deklaratsiya tartibi qo‘llanadi. Ushbu ikki справочникda aniq summa limiti ko‘rsatilmagan, shu sabab aniq javob uchun alohida valyuta qoidalarini tekshirish yoki mutaxassisga murojaat qilish kerak."
    )


def physical_entry_mode(text: str) -> str:
    q = normalize_text(text)
    if any(x in q for x in ["аэропорт", "airport", "aeroport", "самолет", "самолёт", "avia", "авиа", "учу", "лет", "рейс"]):
        return "airport"
    if any(x in q for x in ["границ", "chegara", "пеш", "авто", "машин", "на машине", "поезд", "temir yo'l", "temir yol", "rail"]):
        return "border"
    return ""


PHYSICAL_RULE_TOPICS = {
    "phones", "appliance", "electronics_limit", "medicine", "food", "alcohol", "tobacco", "perfume", "currency"
}


def physical_rule_topic_from_text(text: str) -> str:
    q = normalize_text(text)

    phone_words = ["телефон", "смартфон", "iphone", "айфон", "telefon", "phone", "mobil", "мобильн"]
    appliance_words = [
        "телевиз", "tv", "смарт тв", "smart tv", "холодиль", "sovutgich", "muzlatgich",
        "kondits", "кондиц", "kir yuv", "стирал", "chang yut", "пылесос",
        "gaz plita", "газовая плита", "электроплита", "электро плита", "mikroto", "микроволн",
        "духов", "go'sht qiymalag", "мясоруб", "утюг", "dazmol", "fen"
    ]
    medicine_words = ["лекар", "dori", "tablet", "таблет", "анальгин", "ибупрофен", "парацетамол", "цитрамон", "prep", "препарат"]
    food_words = ["рис", "guruch", "хлеб", "нон", "булк", "мяс", "go'sht", "гушт", "сахар", "shakar", "масло", "egi", "овощ", "sabzavot", "фрукт", "meva"]
    alcohol_words = ["алког", "araq", "vodka", "водка", "вино", "pivo", "пиво", "коньяк", "whisky", "виски"]
    tobacco_words = ["сигар", "tamaki", "табак", "tobacco", "cigar", "вейп", "vape", "snus", "снюс"]
    perfume_words = ["дух", "атир", "parfyum", "парфюм", "туалетн", "cologne"]
    currency_words = ["валют", "valyuta", "доллар", "usd", "евро", "eur", "налич", "cash", "naqd"]

    if any(x in q for x in phone_words):
        return "phones"
    if any(x in q for x in appliance_words):
        return "appliance"
    if any(x in q for x in medicine_words):
        return "medicine"
    if any(x in q for x in food_words):
        return "food"
    if any(x in q for x in alcohol_words):
        return "alcohol"
    if any(x in q for x in tobacco_words):
        return "tobacco"
    if any(x in q for x in perfume_words):
        return "perfume"
    if any(x in q for x in currency_words):
        return "currency"
    if any(x in q for x in ["техник", "tehnik", "электрон", "elektron", "ноутбук", "noutbuk", "laptop", "planshet", "планшет"]):
        return "electronics_limit"
    return ""


def physical_food_item_key(text: str) -> str:
    q = normalize_text(text)
    if "рис" in q or "guruch" in q:
        return "rice"
    if "хлеб" in q or "булк" in q or "нон" in q:
        return "bread"
    if "мяс" in q or "go'sht" in q or "гушт" in q:
        return "meat"
    if "сахар" in q or "shakar" in q:
        return "sugar"
    if "масло" in q or "egi" in q:
        return "oil"
    if any(x in q for x in ["овощ", "sabzavot", "фрукт", "meva", "узум", "uzum"]):
        return "fruitveg"
    return ""


def physical_appliance_item_key(text: str) -> str:
    q = normalize_text(text)
    appliance_map = [
        ("tv", ["телевиз", "tv", "смарт тв", "smart tv"]),
        ("fridge", ["холодиль", "sovutgich"]),
        ("freezer", ["muzlatgich", "морозиль"]),
        ("ac", ["kondits", "кондиц"]),
        ("washer", ["kir yuv", "стирал"]),
        ("vacuum", ["chang yut", "пылесос"]),
        ("gas_stove", ["gaz plita", "газовая плита"]),
        ("electric_stove", ["электроплита", "электро плита", "elektr plita"]),
        ("microwave", ["mikroto", "микроволн"]),
        ("oven", ["духов", "duxovka"]),
        ("grinder", ["go'sht qiymalag", "мясоруб"]),
        ("iron", ["утюг", "dazmol"]),
        ("dryer", ["fen"]),
    ]
    for key, variants in appliance_map:
        if any(v in q for v in variants):
            return key
    return ""


def physical_rule_base_answer(text: str, lang: str, topic: str = "") -> str:
    topic = topic or physical_rule_topic_from_text(text)
    transport = physical_entry_mode(text)
    food_item = physical_food_item_key(text)
    appliance_item = physical_appliance_item_key(text)

    if topic == "phones":
        if transport == "airport":
            return (
                "Aviatsiya punktlari orqali telefon apparati, shu jumladan mobil qurilmalar — har bir kirishda 2 donagacha."
                if lang == "uz" else
                "Через авиапункты телефонные аппараты, включая мобильные устройства, — до 2 штук за один въезд."
            )
        return (
            "Telefonlar bo'yicha aviatsiya punktlarida norma — har bir kirishda 2 dona. Boshqa punktlarda esa olib kirish qiymati transport turiga qarab baholanadi: aeroport — 1000 USD, temir yo'l va daryo — 500 USD, avto/piyoda — 300 USD."
            if lang == "uz" else
            "По телефонам через авиапункты норма — до 2 штук за один въезд. По другим пунктам ориентируются на стоимостной лимит по способу ввоза: аэропорт — 1000 USD, ж/д и речные — 500 USD, авто/пешком — 300 USD."
        )

    if topic == "appliance" or appliance_item:
        if transport == "airport":
            return (
                "Aeroport orqali shaxsiy foydalanish uchun olib kirishda umumiy qiymat limiti 1000 USD. Aniq texnika bo'yicha 6 oylik dona normasi asosan avtoyo'l, temir yo'l va daryo punktlari uchun qo'llanadi."
                if lang == "uz" else
                "Через аэропорт для личного пользования действует общий стоимостной лимит 1000 USD. Количественная норма 1 штука в 6 месяцев по технике в основном указана для авто, ж/д и речных пунктов."
            )
        item_label_ru = {
            "tv": "Телевизор", "fridge": "Холодильник", "freezer": "Морозильник", "ac": "Кондиционер",
            "washer": "Стиральная машина", "vacuum": "Пылесос", "gas_stove": "Газовая плита",
            "electric_stove": "Электроплита", "microwave": "Микроволновая печь", "oven": "Духовка",
            "grinder": "Электромясорубка", "iron": "Утюг", "dryer": "Фен"
        }.get(appliance_item, "Техника")
        item_label_uz = {
            "tv": "Televizor", "fridge": "Sovutgich", "freezer": "Muzlatgich", "ac": "Konditsioner",
            "washer": "Kir yuvish mashinasi", "vacuum": "Chang yutgich", "gas_stove": "Gaz plitasi",
            "electric_stove": "Elektr plita", "microwave": "Mikroto'lqinli pech", "oven": "Duxovkali pech",
            "grinder": "Elektr go'sht qiymalagich", "iron": "Dazmol", "dryer": "Fen"
        }.get(appliance_item, "Texnika")
        return (
            f"{item_label_uz}: avtoyo'l, temir yo'l va daryo punktlari orqali — 6 kalendar oyda 1 dona."
            if lang == "uz" else
            f"{item_label_ru}: через авто, ж/д и речные пункты — 1 штука в 6 календарных месяцев."
        )

    if topic == "electronics_limit":
        return (
            "Shaxsiy foydalanish uchun olib kirishda qiymat limiti transport turiga bog'liq: aeroport — 1000 USD, temir yo'l va daryo — 500 USD, avto/piyoda — 300 USD, kuryer — 200 USD, xalqaro pochta — 100 USD."
            if lang == "uz" else
            "При ввозе для личного пользования стоимостной лимит зависит от транспорта: аэропорт — 1000 USD, ж/д и речные пункты — 500 USD, авто/пешком — 300 USD, курьер — 200 USD, международная почта — 100 USD."
        )

    if topic == "medicine":
        return (
            "Dori vositalari: suyuq holda sirop — 10 flakongacha, kukun — 0,5 kg, tabletkalar — 100 dona, kapsula yoki draje — 100 dona, ampula — 10 donagacha, quritilgan o'simliklar — 0,5 kg."
            if lang == "uz" else
            "Лекарства: сиропы — до 10 флаконов, порошки — до 0,5 кг, таблетки — до 100 штук, капсулы или драже — до 100 штук, ампулы — до 10 штук, сушёные растения — до 0,5 кг."
        )

    if topic == "food":
        food_answers_ru = {
            "rice": "Рис — до 3 кг.",
            "bread": "Хлебобулочные изделия — до 5 кг.",
            "meat": "Мясо и мясопродукты — до 2 кг.",
            "sugar": "Сахар — до 2 кг.",
            "oil": "Растительное масло — до 2 кг.",
            "fruitveg": "Свежие и сушёные овощи, фрукты, виноград, бахчевые и бобовые — до 40 кг.",
        }
        food_answers_uz = {
            "rice": "Guruch — 3 kg gacha.",
            "bread": "Non-bulka mahsulotlari — 5 kg gacha.",
            "meat": "Go'sht va go'sht mahsulotlari — 2 kg gacha.",
            "sugar": "Shakar — 2 kg gacha.",
            "oil": "O'simlik yog'i — 2 kg gacha.",
            "fruitveg": "Yangi uzilgan va quritilgan meva-sabzavotlar, uzum, poliz va dukkakli mahsulotlar — 40 kg gacha.",
        }
        if food_item:
            return food_answers_uz[food_item] if lang == "uz" else food_answers_ru[food_item]
        return (
            "Oziq-ovqat bo'yicha norma mahsulot turiga qarab belgilanadi. Masalan: guruch — 3 kg, non-bulka — 5 kg, go'sht — 2 kg, shakar — 2 kg, o'simlik yog'i — 2 kg, meva-sabzavotlar — 40 kg."
            if lang == "uz" else
            "По продуктам норма зависит от вида товара. Например: рис — 3 кг, хлебобулочные — 5 кг, мясо — 2 кг, сахар — 2 кг, растительное масло — 2 кг, овощи и фрукты — 40 кг."
        )

    if topic == "alcohol":
        return (
            "Alkogolli ichimliklar — 2 litrgacha."
            if lang == "uz" else
            "Алкогольные напитки — до 2 литров."
        )

    if topic == "tobacco":
        return (
            "Tamaki mahsulotlari: sigaret — 200 dona, sigara — 5 dona, tamaki — 100 grammgacha."
            if lang == "uz" else
            "Табачные изделия: сигареты — до 200 штук, сигары — до 5 штук, табак — до 100 граммов."
        )

    if topic == "perfume":
        return (
            "Atir-upa mahsulotlari — 3 donagacha, umumiy hajmi 300 ml dan oshmasligi kerak."
            if lang == "uz" else
            "Парфюмерия — до 3 единиц, общий объём не более 300 мл."
        )

    if topic == "currency":
        return (
            "Valyuta bo'yicha alohida amaldagi deklaratsiya tartibi qo'llanadi. Summani va olib kirishmi yoki olib chiqishmi — shuni yozing."
            if lang == "uz" else
            "По валюте действует отдельный порядок декларирования. Напишите сумму и уточните: ввоз или вывоз."
        )
    return ""


def physical_should_use_rule_answer(text: str, c: Dict[str, Any]) -> str:
    topic = physical_rule_topic_from_text(text)
    if topic:
        return topic
    current = physical_rule_topic(c) or physical_get_pro_topic(c)
    q = normalize_text(text)
    if current:
        short_follow = len(q.split()) <= 8 or bool(physical_entry_mode(text)) or any(ch.isdigit() for ch in text)
        if short_follow:
            return current
    return ""


def physical_should_use_ai_followup_v2(text: str, c: Dict[str, Any]) -> bool:
    q = normalize_text(text)
    topic = physical_rule_topic(c) or physical_get_pro_topic(c)
    if not topic:
        return False
    if looks_like_new_question(text):
        return False
    if is_legal_commercial_question(text):
        return False
    detected = physical_rule_topic_from_text(text)
    if detected and detected != topic:
        return False
    if len(q.split()) <= 12:
        return True
    if physical_entry_mode(text):
        return True
    if any(ch.isdigit() for ch in text):
        return True
    return False
...
def physical_pro_global_hint(text: str, lang: str) -> str:
    ...
    if not isinstance(rules, dict):
        return ""
    if topic == "currency":
        return str(rules.get("currency", "") or "").strip()
    if topic == "medicine":
        return str(rules.get("med", "") or rules.get("medicine", "") or "").strip()
    if topic in {"import", "phones", "notebook", "tv", "tobacco_alcohol", "perfume", "jewelry", "forbidden", "pyro", "drone"}:
        return str(rules.get("import", "") or "").strip()
    if topic == "auto":
        return str(rules.get("auto", "") or "").strip()
    return ""


def build_physical_ai_prompt(user_text: str, c: Dict[str, Any], lang: str) -> str:
    topic = physical_rule_topic(c) or physical_get_pro_topic(c) or physical_detect_topic_plus(user_text, lang)
    direction = physical_detect_direction(user_text, lang)
    transport = physical_detect_transport(user_text, lang)
    local = []
    rule_source = c.get("physical_rule_state", {}).get("source_text", "") or user_text
    rule_base = physical_rule_base_answer(rule_source, lang, physical_rule_topic(c))
    if rule_base:
        local.append(rule_base)
    patt = physical_pro_pattern_answer(user_text, lang)
    if patt and patt not in local:
        local.append(patt)
    ...
    if lang == "uz":
        base = (
            ...
            "Mahalliy kontekstda berilgan norma va raqamlardan chiqma. "
            "Agar mahalliy kontekstda aniq limit bo‘lmasa, yangi limit o‘ylab topma, faqat yetishmayotgan ma'lumotni so‘ra yoki mutaxassisga yo‘naltir. "
            "Javobni 2–4 gapda, qisqa va aniq yoz."
        )
    else:
        base = (
            ...
            "Не выдумывай новые лимиты, проценты, количество и суммы. "
            "Используй только те нормы и цифры, которые есть в локальном контексте ниже. "
            "Если в локальном контексте точной нормы нет — не подставляй её по памяти, а попроси уточнение или отправь к специалисту. "
            "Отвечай коротко, по делу, 2–4 предложениями."
        )
...
def build_physical_doc_first_answer(uid: int, text: str, lang: str):
    c = ctx(uid)
    if not PHYSICAL_ROUTER or not PHYSICAL_ENGINE:
        fallback = physical_answer_legacy(text, lang)
        return fallback, {"category_score": 0, "category": "general_limits", "item": None, "intent": "basic_permission", "facts": {}, "lang": lang}

    prior = get_physical_context(c)
    route = PHYSICAL_ROUTER.route(text, context=prior)
    answer = PHYSICAL_ENGINE.build_answer(route).strip()

    extra = t(lang, "physical_free_specialist") if "physical_free_specialist" in TXT.get(lang, {}) else t(lang, "free_specialist")
    if extra:
        answer = answer + "\n\n" + extra

    save_physical_context(c, route, answer)
    return answer, route


def physical_doc_should_use_ai(route: Dict[str, Any]) -> bool:
    score = int(route.get("category_score", 0) or 0)
...
async def start_cmd(message: types.Message):
    USER_CTX[message.from_user.id] = {"lang":"ru","role":None,"mode":"choose_lang","category":None,"group":None,"pending_form":None,"form_data":{},"physical_pro_context":{}}
...
async def router(message: types.Message):
    ...
    if text in ["Русский", "O'zbekcha", "O‘zbekcha", "🇷🇺 Русский", "🇺🇿 O'zbekcha", "🇺🇿 O‘zbekcha"]:
        c["lang"] = "ru" if "Русский" in text else "uz"
        c["mode"] = "choose_role"
    ...
    if role == "physical":
        ...
        for _ready_key in ["physical_q1", "physical_q2", "physical_q3", "physical_q4", "physical_q5", "physical_q6"]:
            if button_eq(text, t(lang, _ready_key)):
                ...
                physical_clear_rule_state(c)
                c["mode"] = "physical_chat"
                rule_topic = physical_rule_topic_from_text(text)
                if rule_topic:
                    physical_set_rule_topic(c, rule_topic, text)
                    physical_set_pro_topic(c, rule_topic, text)
                    await message.answer(physical_rule_base_answer(text, lang, rule_topic), reply_markup=physical_ai_kb(lang))
                    return
                physical_set_pro_topic(c, physical_pro_guess_topic(text), text)
                answer, route = build_physical_doc_first_answer(uid, text, lang)
    ...
    if c["mode"] == "physical_ready":
        rule_topic = physical_rule_topic_from_text(text)
        if rule_topic:
            base_answer = physical_rule_base_answer(text, lang, rule_topic)
            if base_answer:
                physical_clear_state(c)
                ...
                physical_set_pro_topic(c, rule_topic, text)
                c["mode"] = "physical_chat"
                await message.answer(base_answer, reply_markup=physical_ai_kb(lang))
                return
    ...
    if c["mode"] == "physical_chat":
        ...
        if any(marker in normalize_text(text) for marker in commercial_markers):
            physical_clear_state(c)
            physical_clear_pro_state(c)
            physical_clear_rule_state(c)
            clear_physical_context(c)
        ...
        rule_topic = physical_should_use_rule_answer(text, c)
        current_rule_topic = physical_rule_topic(c)

        if rule_topic and (looks_like_new_question(text) or not current_rule_topic or rule_topic != current_rule_topic):
            base_answer = physical_rule_base_answer(text, lang, rule_topic)
            if base_answer:
                physical_clear_state(c)
                physical_clear_pro_state(c)
        ...
        if current_rule_topic == "currency":
            currency_answer = physical_currency_followup_answer(c, text, lang)
            if currency_answer:
                await message.answer(currency_answer, reply_markup=physical_ai_kb(lang))
                return

        if current_rule_topic and physical_should_use_ai_followup_v2(text, c):
            ai_text = physical_ai_answer(text, c, lang)
            if ai_text:
                await send_safe_message(message, ai_text, reply_markup=physical_ai_kb(lang))
                return
            fallback = physical_rule_base_answer(text, lang, current_rule_topic)
            if fallback:
                await message.answer(fallback, reply_markup=physical_ai_kb(lang))
                return

        answer, route = build_physical_doc_first_answer(uid, text, lang)

        if not physical_doc_should_use_ai(route):
            physical_clear_state(c)
            physical_clear_rule_state(c)
            if route.get("item") or route.get("category") != "general_limits":
                physical_set_pro_topic(c, route.get("item") or route.get("category") or physical_pro_guess_topic(text), text)
            await send_safe_message(message, answer, reply_markup=physical_ai_kb(lang))
            return
        ...
        if physical_needs_ai_followup(text, c, lang):
            ...
            if ai_text:
                physical_clear_state(c)
                await send_safe_message(message, ai_text + "\n\n" + t(lang, "physical_free_specialist"), reply_markup=physical_ai_kb(lang))
                return

        if not looks_like_customs_question(text) and not looks_like_physical_customs_question(text):
            ai_text = physical_ai_answer(text, c, lang)
            if ai_text:
                await send_safe_message(message, ai_text, reply_markup=physical_ai_kb(lang))
                return
