...
def ai_hint(query: str, items: List[Dict[str, Any]], lang: str) -> str:
    ...
    try:
        ...
        system = (
            "Ты помощник по таможне Узбекистана. Отвечай кратко, точно и без выдуманных кодов. Используй только переданные данные."
            if lang == "ru" else
            "Sen O‘zbekiston bojxona yordamchisisan. Faqat berilgan ma’lumotlarga tayangan holda qisqa va aniq javob ber."
        )
        user_prompt = (
...
PHYSICAL_RULES = {
    "ru": {
        "import_limits": "Беспошлинный ввоз для физлиц зависит от способа пересечения границы: аэропорт — до 2000 долларов США, железнодорожные и речные пункты — до 1000 долларов США, автодорожные и пешеходные пункты — до 300 долларов США.",
        "special_import": "Отдельные нормы беспошлинного ввоза: алкоголь — до 2 литров, все виды табачных изделий — до 10 пачек, духи и туалетная вода — до 3 единиц, ювелирные изделия — до 65 граммов.",
        "land_goods": "Через автодорожные, пешеходные, железнодорожные и речные пункты для ряда товаров действуют количественные нормы. Например, холодильник, телевизор, стиральная машина, ноутбук, планшет, компьютерная техника и телефонный аппарат — по 1 единице на 6 календарных месяцев на одно физлицо.",
        "air_phones": "При авиапересечении для телефонных аппаратов, включая мобильные телефоны и устройства с модулем мобильной связи, указана норма до 2 единиц при каждом въезде.",
        "export_limits": "При выезде из Узбекистана физлицу разрешается вывозить товары общей стоимостью не более 5000 долларов США без подачи таможенной декларации.",
        "export_food": "Отдельные нормы вывоза: рис — до 3 кг, хлебобулочные изделия — до 5 кг, мясо и пищевые мясные субпродукты — до 2 кг, сахар — до 2 кг, растительные масла — до 2 кг, свежая плодоовощная продукция и аналогичные товары — до 40 кг.",
        "currency": "Ввоз наличной валюты физлицами в Узбекистан не ограничен. Вывоз наличной валюты физлицами за пределы Узбекистана допускается в сумме не более эквивалента 100 000 000 сумов. Сверх этой суммы действует отдельный порядок.",
        "medicines": "Для личного пользования без меддокумента допускается ввоз и вывоз до 10 лекарственных препаратов разного наименования и не более 5 упаковок каждого, а медизделий — не более 5 единиц. Лекарства должны быть в заводской упаковке. Для наркотических и психотропных веществ действуют отдельные, более строгие правила.",
        "vehicles": "По автотранспорту действует отдельный режим. Общая продолжительность временного ввоза транспортного средства для некоммерческих целей обычно не должна превышать 90 календарных дней в течение календарного года, если иное не оформлено в установленном порядке.",
        "pyro": "Пиротехнические средства относятся к чувствительной категории и по ним действуют серьёзные ограничения. По таким товарам нужно сразу предупреждать о запретах и специальных требованиях.",
        "commercial": "Если количество однородных товаров превышает обычную потребность физлица и членов его семьи, таможня может рассматривать такой ввоз как связанный с предпринимательской деятельностью, если лицо не докажет обратное."
    },
    "uz": {
        "import_limits": "Jismoniy shaxslar uchun bojsiz olib kirish limiti kirish usuliga bog‘liq: aeroport orqali — 2000 AQSH dollarigacha, temir yo‘l va daryo punktlari orqali — 1000 AQSH dollarigacha, avtoyo‘l va piyoda punktlari orqali — 300 AQSH dollarigacha.",
        "special_import": "Alohida bojsiz me’yorlar: alkogol — 2 litrgacha, tamaki mahsulotlari — 10 pachkagacha, atir va tualet suvi — 3 birlikkacha, zargarlik buyumlari — 65 grammgacha.",
        "land_goods": "Avtoyo‘l, piyoda, temir yo‘l va daryo punktlari orqali ayrim tovarlar uchun miqdoriy me’yorlar bor. Masalan, muzlatgich, televizor, kir yuvish mashinasi, noutbuk, planshet, kompyuter texnikasi va telefon apparati — 6 kalendar oyda 1 donagacha.",
        "air_phones": "Aviatsiya orqali kirishda telefon apparatlari, jumladan mobil telefonlar va mobil aloqa moduliga ega qurilmalar uchun har bir kirishda 2 donagacha norma ko‘rsatilgan.",
        "export_limits": "O‘zbekistondan chiqishda jismoniy shaxs bojxona deklaratsiyasisiz umumiy qiymati 5000 AQSH dollaridan oshmaydigan tovarlarni olib chiqishi mumkin.",
        "export_food": "Alohida olib chiqish me’yorlari: guruch — 3 kg gacha, non mahsulotlari — 5 kg gacha, go‘sht — 2 kg gacha, shakar — 2 kg gacha, o‘simlik yog‘i — 2 kg gacha, meva-sabzavot va o‘xshash mahsulotlar — 40 kg gacha.",
        "currency": "Naqd valyutani O‘zbekistonga olib kirish cheklanmagan. O‘zbekistondan olib chiqish esa 100 000 000 so‘m ekvivalentidan oshmagan miqdorda ruxsat etiladi. Undan yuqori summa uchun alohida tartib bor.",
        "medicines": "Shaxsiy foydalanish uchun tibbiy hujjatsiz 10 xil nomdagi dori vositasigacha va har biridan 5 qadoqdan oshmagan miqdorda, tibbiy buyumlar esa 5 donagacha olib o‘tilishi mumkin. Dorilar ishlab chiqaruvchi qadoqlarida bo‘lishi kerak. Narkotik va psixotrop moddalar uchun alohida qat’iy qoidalar bor.",
        "vehicles": "Avtotransport bo‘yicha alohida tartib mavjud. Nokommersiya maqsadida vaqtincha olib kirilgan transport vositasining umumiy muddati odatda bir kalendar yil davomida 90 kundan oshmasligi kerak, agar qonuniy tartibda boshqacha rasmiylashtirilmagan bo‘lsa.",
        "pyro": "Pirotexnika bo‘yicha jiddiy cheklovlar mavjud. Bunday tovarlar uchun darhol taqiq va maxsus talablar haqida ogohlantirish kerak.",
        "commercial": "Bir xil tovarlar soni jismoniy shaxs va uning oilasi ehtiyojidan ortiq bo‘lsa, bojxona buni tijorat maqsadidagi olib kirish deb baholashi mumkin, agar buning aksi isbotlanmasa."
    }
}


def _rules(lang: str) -> Dict[str, str]:
    return PHYSICAL_RULES["uz" if lang == "uz" else "ru"]
...
def extract_money_info(text: str) -> Dict[str, Any]:
    ...
    if m:
        ...
        try:
            ...
            amount = None
    currency = None
    if any(x in q for x in ["доллар", "usd", "долл"]):
        currency = "USD"
    elif "евро" in q or "eur" in q:
        currency = "EUR"
    elif any(x in q for x in ["сум", "uzs", "so'm", "som", "soʻm"]):
        currency = "UZS"
...
def is_currency_question(text: str) -> bool:
    q = normalize_text(text)
    if any(x in q for x in ["валют", "valyut", "доллар", "usd", "евро", "eur", "сум", "uzs", "so'm", "som", "soʻm"]):
        return True
    info = extract_money_info(text)
    return info["amount"] is not None and info["currency"] is not None


def is_medicine_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["лекар", "таблет", "капсул", "ампул", "укол", "инсулин", "медицин", "dori", "tablet", "kapsul", "ampula", "tibbiy"])


def is_vehicle_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["авто", "машин", "автомоб", "tesla", "byd", "transport", "mashina", "avto"])


def is_pyro_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["пиротех", "салют", "фейер", "петард", "pirotex", "salyut"])


def is_phone_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["телефон", "iphone", "айфон", "смартфон", "phone", "telefon"])


def is_export_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["вывоз", "вывез", "выез", "olib chiq", "chiq", "export"])


def is_import_question(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["ввоз", "ввез", "въез", "привез", "провез", "olib kir", "import", "kir"])


def is_air_route(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["аэропорт", "самолет", "самолёт", "airport", "aeroport", "avia", "aviatsiya"])


def is_land_route(text: str) -> bool:
    q = normalize_text(text)
    return any(x in q for x in ["машин", "авто", "пеш", "границ", "поезд", "жд", "ж/д", "автодорож", "mashina", "avto", "piyoda", "temir yo'l"])
...
def physical_answer(q: str, lang: str) -> str:
    ...
    if is_currency_question(q):
        info = extract_money_info(q)
        if lang == "ru":
            if is_export_question(q) or "вывезти" in qn or info["currency"]:
                if info["amount"] is not None and info["currency"]:
                    amt = int(info["amount"]) if float(info["amount"]).is_integer() else info["amount"]
                    return (
                        f"Физлицам разрешён вывоз наличной валюты из Узбекистана в сумме не более эквивалента 100 000 000 сумов. "
                        f"{amt} {info['currency']} нужно оценивать относительно этого лимита. Если сумма выше предела, применяется отдельный порядок. Вы планируете вывоз наличными или через банк?"
                    )
                return kb["currency"] + " Напишите, какую именно валюту и сумму вы хотите ввезти или вывезти."
            return kb["currency"]
        else:
            if is_export_question(q) or info["currency"]:
                if info["amount"] is not None and info["currency"]:
                    amt = int(info["amount"]) if float(info["amount"]).is_integer() else info["amount"]
                    return (
                        f"Jismoniy shaxslar uchun naqd valyutani olib chiqish 100 000 000 so‘m ekvivalentigacha ruxsat etiladi. "
                        f"{amt} {info['currency']} aynan shu limitga nisbatan baholanadi. Agar summa yuqori bo‘lsa, alohida tartib qo‘llanadi. Naqd olib chiqmoqchimisiz yoki bank orqali?"
                    )
                return kb["currency"] + " Qaysi valyuta va qancha summa haqida gap ketayotganini yozing."
            return kb["currency"]

    if is_medicine_question(q):
        return (
            f"Да, для личного пользования лекарства ввозить и вывозить можно, но действуют количественные нормы. {kb['medicines']} Напишите название лекарства, количество, форму выпуска и есть ли рецепт или медсправка."
            if lang == "ru" else
            f"Ha, shaxsiy foydalanish uchun dorilarni olib kirish va olib chiqish mumkin, lekin miqdoriy me’yorlar bor. {kb['medicines']} Dori nomi, miqdori, shakli va retsept yoki tibbiy ma’lumotnoma bor-yo‘qligini yozing."
        )

    if is_pyro_question(q):
        return (
            f"Пиротехника — чувствительная категория. {kb['pyro']} Напишите точное название товара, чтобы я сориентировал точнее."
            if lang == "ru" else
            f"Pirotexnika sezgir toifa. {kb['pyro']} Aniq mahsulot nomini yozing, men aniqroq yo‘naltiraman."
        )

    if is_vehicle_question(q):
        return (
            f"По автомобилям для физлиц действует отдельный порядок. {kb['vehicles']} Уточните: это временный ввоз, временный вывоз, покупка новой машины или въезд на иностранном авто?"
            if lang == "ru" else
            f"Avtomobillar bo‘yicha jismoniy shaxslar uchun alohida tartib bor. {kb['vehicles']} Aniqlashtiring: bu vaqtincha olib kirishmi, vaqtincha olib chiqishmi, yangi mashina sotib olishmi yoki xorijiy avtomobilda kirishmi?"
        )

    if is_phone_question(q):
        if is_air_route(q):
            return (
                f"Если ввоз через аэропорт, сначала смотрят общий лимит по способу въезда, потом специальную норму по телефонам и признаки личного пользования. {kb['import_limits']} {kb['air_phones']} {kb['commercial']} Напишите, сколько телефонов, они новые или б/у, и для себя ли вы их везёте."
                if lang == "ru" else
                f"Agar aeroport orqali olib kirilsa, avval kirish usuli bo‘yicha umumiy limit, keyin telefonlar bo‘yicha maxsus me’yor va shaxsiy foydalanish alomatlari ko‘riladi. {kb['import_limits']} {kb['air_phones']} {kb['commercial']} Nechta telefon, yangi yoki ishlatilganmi va o‘zingiz uchunmi — shuni yozing."
            )
        return (
            f"По телефону для физлица важно учитывать способ въезда, количество и признаки личного пользования. {kb['import_limits']} {kb['land_goods']} {kb['commercial']} Напишите, сколько телефонов вы везёте и через что въезжаете: аэропорт, авто, поезд или пешком."
            if lang == "ru" else
            f"Telefon bo‘yicha jismoniy shaxs uchun kirish usuli, soni va shaxsiy foydalanish alomatlari muhim. {kb['import_limits']} {kb['land_goods']} {kb['commercial']} Nechta telefon olib kirayotganingizni va qaysi yo‘l bilan kirayotganingizni yozing: aeroport, avto, poyezd yoki piyoda."
        )

    if is_export_question(q):
        return (
            f"Если вопрос про вывоз, сначала смотрят общий лимит по стоимости, потом — отдельные нормы на конкретные товары и чувствительные категории. {kb['export_limits']} {kb['export_food']} Если товар относится к лекарствам, культурным ценностям, оружию, растениям или животным, могут понадобиться разрешения. Напишите, что именно вы хотите вывезти."
            if lang == "ru" else
            f"Agar savol olib chiqish haqida bo‘lsa, avval umumiy qiymat limiti, keyin aniq tovar bo‘yicha alohida me’yorlar va sezgir toifalar tekshiriladi. {kb['export_limits']} {kb['export_food']} Agar tovar dori, madaniy boylik, qurol, o‘simlik yoki hayvonlarga oid bo‘lsa, ruxsat talab qilinishi mumkin. Aynan nimani olib chiqmoqchi ekaningizni yozing."
        )

    if is_import_question(q):
        extra = kb['land_goods'] if is_land_route(q) else kb['special_import']
        return (
            f"Если вопрос про ввоз, я сначала смотрю лимит по способу пересечения границы, потом отдельную норму по товару и после этого оцениваю, похоже ли это на личное пользование или на коммерцию. {kb['import_limits']} {extra} {kb['commercial']} Напишите товар, количество и через что въезжаете: аэропорт, авто, поезд или пешком."
            if lang == "ru" else
            f"Agar savol olib kirish haqida bo‘lsa, avval chegarani kesib o‘tish usuli bo‘yicha limit, keyin tovar bo‘yicha alohida me’yor va undan keyin bu shaxsiy foydalanishmi yoki tijoratga o‘xshaydimi — shuni baholayman. {kb['import_limits']} {extra} {kb['commercial']} Tovar nomi, miqdori va qaysi yo‘l bilan kirayotganingizni yozing: aeroport, avto, poyezd yoki piyoda."
        )

    if any(x in qn for x in ["сколько", "qancha", "nechta", "лимит", "норма", "me'yor"]):
        return (
            f"Для физлиц ответ зависит от товара и способа пересечения границы. {kb['import_limits']} {kb['land_goods']} Если это вывоз, то {kb['export_limits']} Напишите конкретный товар и маршрут."
            if lang == "ru" else
            f"Jismoniy shaxslar uchun javob tovar va chegarani kesib o‘tish usuliga bog‘liq. {kb['import_limits']} {kb['land_goods']} Agar olib chiqish bo‘lsa, {kb['export_limits']} Aniq tovar va yo‘nalishni yozing."
        )

    return (
        "Я помогу по таможенным правилам для физлиц. Напишите товар, количество и как именно пересекаете границу — аэропорт, авто, поезд или пешком."
        if lang == "ru" else
        "Jismoniy shaxslar uchun bojxona qoidalari bo‘yicha yordam beraman. Tovar, miqdor va chegarani qanday kesib o‘tayotganingizni yozing: aeroport, avto, poyezd yoki piyoda."
    )
...
def is_physical_rate_question(q: str) -> bool:
    qn = normalize_text(q)
    strict_markers = [
        "ставка", "ставки", "пошлина", "пошлины", "код тн вэд", "тн вэд", "tn ved",
        "ндс", "акциз", "утиль", "qqs", "aksiz", "util", "код товара", "gtd", "гтд",
        "сертификат соответствия", "контракт", "invoice", "invoys", "брокер", "broker"
    ]
    if any(m in qn for m in strict_markers):
        return True
    if any(m in qn for m in ["для продажи", "коммер", "оптом", "контейнер", "partiya", "ulgurji", "tijorat"]):
        return True
    return False
...
def physical_followup_answer(topic: str, detail: str, lang: str, state: Dict[str, Any] = None) -> str:
    ...
    if topic == "currency":
        info = extract_money_info(item if item else product)
        if lang == "ru":
            if info["amount"] is not None and info["currency"]:
                amt = int(info["amount"]) if float(info["amount"]).is_integer() else info["amount"]
                return (
                    f"Физлицам разрешён вывоз наличной валюты из Узбекистана в сумме не более эквивалента 100 000 000 сумов. "
                    f"{amt} {info['currency']} нужно оценивать относительно этого лимита. Если сумма выше, действует отдельный порядок. "
                    f"Если речь о ввозе, то ввоз наличной валюты не ограничен."
                )
            return kb["currency"] + " Напишите, какую валюту и какую сумму вы хотите ввезти или вывезти."
        else:
            if info["amount"] is not None and info["currency"]:
                amt = int(info["amount"]) if float(info["amount"]).is_integer() else info["amount"]
                return (
                    f"Jismoniy shaxslar uchun naqd valyutani olib chiqish 100 000 000 so‘m ekvivalentigacha ruxsat etiladi. "
                    f"{amt} {info['currency']} aynan shu limitga nisbatan baholanadi. Agar summa yuqori bo‘lsa, alohida tartib qo‘llanadi. "
                    f"Agar gap olib kirish haqida bo‘lsa, naqd valyutani olib kirish cheklanmagan."
                )
            return kb["currency"] + " Qaysi valyuta va qancha summa haqida gap ketayotganini yozing."
    ...
    if topic == "medicine":
        if not product:
            return (
                "Лекарства для личного пользования можно ввозить и вывозить, но сначала нужно понять, что именно за препарат. Напишите название лекарства."
                if lang == "ru" else
                "Dorilarni shaxsiy foydalanish uchun olib kirish va olib chiqish mumkin, lekin avval aynan qaysi preparat ekanini bilish kerak. Dori nomini yozing."
            )
        if product and not quantity:
            return (
                f"По лекарству «{product}» теперь нужен объём. Напишите, сколько упаковок и какая форма выпуска."
                if lang == "ru" else
                f"«{product}» dorisi bo‘yicha endi miqdor kerak. Nechta qadoq va qaysi shaklda ekanini yozing."
            )
        if product and quantity and not country:
            return (
                f"По лекарству «{product}» в количестве {quantity} для личного пользования действуют количественные нормы. {kb['medicines']} Теперь напишите, из какой страны ввозите или вывозите."
                if lang == "ru" else
                f"«{product}» dorisini {quantity} miqdorda shaxsiy foydalanish uchun olib o‘tishda miqdoriy me’yorlar bor. {kb['medicines']} Endi qaysi davlatdan olib kirish yoki olib chiqish haqida gap ketayotganini yozing."
            )
        return (
            f"По лекарству «{product}» в количестве {quantity} из {country} обычно смотрят состав, разумный объём и то, не относится ли препарат к наркотическим или психотропным веществам. {kb['medicines']}"
            if lang == "ru" else
            f"«{product}» dorisini {quantity} miqdorda {country}dan olib o‘tishda odatda tarkib, me’yoriy miqdor va preparat narkotik yoki psixotrop toifaga kiradimi — shunga qaraladi. {kb['medicines']}"
        )

    if topic == "phones":
        if not product:
            return (
                "Телефоны для физлица оцениваются по количеству, маршруту и признакам личного пользования. Напишите, какие именно телефоны вы везёте."
                if lang == "ru" else
                "Telefonlar jismoniy shaxs uchun soni, yo‘nalishi va shaxsiy foydalanish alomatlari bo‘yicha baholanadi. Qaysi telefonlarni olib kirayotganingizni yozing."
            )
        if product and not quantity:
            return (
        ...
        if product and quantity and not country:
            return (
                f"По телефону «{product}» в количестве {quantity} сначала смотрят маршрут, потом норму по количеству и признаки личного пользования. {kb['commercial']} Теперь напишите, откуда ввозите и через что въезжаете."
                if lang == "ru" else
                f"«{product}» telefonini {quantity} miqdorda olib kirishda avval yo‘nalish, keyin miqdoriy me’yor va shaxsiy foydalanish alomatlari ko‘riladi. {kb['commercial']} Endi qayerdan va qaysi yo‘l bilan kirayotganingizni yozing."
            )
        return (
            f"По телефону «{product}» в количестве {quantity} из {country} для физлица смотрят маршрут, количество и признаки личного пользования. Если это аэропорт, может применяться норма до 2 устройств на каждый въезд; если автодорога, ж/д или пеший пункт — обычно смотрят количественные нормы и периодичность. {kb['commercial']}"
            if lang == "ru" else
            f"«{product}» telefonini {quantity} miqdorda {country}dan olib kirishda yo‘nalish, son va shaxsiy foydalanish alomatlari ko‘riladi. Agar aeroport bo‘lsa, har kirishda 2 qurilmagacha norma qo‘llanishi mumkin; agar avtoyo‘l, temir yo‘l yoki piyoda punkt bo‘lsa — odatda miqdoriy me’yor va davriylik tekshiriladi. {kb['commercial']}"
        )

    if topic == "norm_exceeded":
        if not product:
            return (
                "Если речь о превышении нормы, сначала нужно понять, какой именно товар вы везёте. Напишите товар."
                if lang == "ru" else
                "Agar gap me’yor oshishi haqida bo‘lsa, avval qaysi tovar ekanini bilish kerak. Tovar nomini yozing."
            )
        if product and not quantity:
            return (
        ...
        return (
            f"Если норма превышена по товару «{product}» в объёме {quantity}, могут применяться таможенные платежи на превышение. Точный ответ зависит от маршрута и стоимости. Напишите, откуда и как вы въезжаете."
            if lang == "ru" else
            f"Agar «{product}» tovari bo‘yicha {quantity} miqdorda me’yor oshsa, ortiqcha qismi uchun bojxona to‘lovlari qo‘llanishi mumkin. Aniq javob yo‘nalish va qiymatga bog‘liq. Qayerdan va qanday kirayotganingizni yozing."
        )

    return (
        "Уточните товар, количество и страну: сейчас данных мало."
        if lang == "ru" else
        "Tovar, miqdor va davlatni aniqlashtiring: hozir ma’lumot yetarli emas."
    )
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        if ready_answer:
            topic = physical_followup_topic(text, lang)
            if topic:
                physical_set_state(c, topic, lang)
                c["mode"] = "physical_chat"
                await message.answer(
        ...
        # If user typed a custom clarification after a ready question, continue in physical_chat
        if c.get("physical_ai_state", {}).get("topic"):
            c["mode"] = "physical_chat"
    ...
    if c["mode"] == "physical_chat":
        ...
        # structured follow-up for physical AI: product -> quantity -> country
        if state.get("topic"):
            ...
            # a real new question resets previous state
            physical_clear_state(c)
