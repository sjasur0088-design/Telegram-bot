...
# PHYSICAL AI PRO MAX
# =========================
def physical_get_ai_state(c: Dict[str, Any]) -> Dict[str, Any]:
    return c.setdefault("physical_ai_followup", {})

def physical_clear_ai_state(c: Dict[str, Any]) -> None:
    c["physical_ai_followup"] = {}

def physical_set_ai_topic(c: Dict[str, Any], topic: str, value: str = "") -> None:
    c["physical_ai_followup"] = {
        "topic": topic,
        "value": (value or "")[:500]
    }

def physical_get_ai_topic(c: Dict[str, Any]) -> str:
    return physical_get_ai_state(c).get("topic", "")

def physical_guess_topic_from_text(text: str) -> str:
    q = normalize_text(text)

    if any(x in q for x in ["лекар", "таблет", "цитрамон", "анальгин", "парацетамол", "ибупрофен", "citramon", "analgin", "paratsetamol", "ibuprofen", "dori"]):
        return "medicines"
    if any(x in q for x in ["телевизор", "tv", "smart tv", "смарт тв", "televizor"]):
        return "tv"
    if any(x in q for x in ["телефон", "айфон", "смартфон", "iphone", "telefon"]):
        return "phone"
    if any(x in q for x in ["ноутбук", "noutbuk", "laptop"]):
        return "notebook"
    if any(x in q for x in ["валют", "доллар", "евро", "сум", "usd", "eur", "dollar", "valyut"]):
        return "currency"
    return ""

def physical_needs_ai_followup(text: str, c: Dict[str, Any], lang: str) -> bool:
    q = normalize_text(text)
    topic = physical_get_ai_topic(c)
    ...
    if topic == "medicines":
        generic_words = {
            "ru": ["можно", "нельзя", "сколько", "лекарство", "лекарства", "ввоз", "вывоз", "таблетки", "упаков", "видов"],
            "uz": ["mumkin", "mumkinmi", "qancha", "dori", "dorilar", "olib kirish", "olib chiqish", "qadoq"]
        }
        bad = generic_words["uz"] if lang == "uz" else generic_words["ru"]
        if 1 <= len(q.split()) <= 4 and not any(w in q for w in bad):
            return True

    if topic in {"tv", "phone", "notebook", "currency"}:
        if 1 <= len(q.split()) <= 6:
            return True
...
def build_physical_ai_prompt(user_text: str, c: Dict[str, Any], lang: str) -> str:
    topic = physical_get_ai_topic(c) or physical_guess_topic_from_text(user_text)

    base_rules_ru = (
        "Ты помощник по таможенным вопросам Узбекистана для физических лиц. "
        "Сначала опирайся на локальные правила. Не путай физлиц и юрлиц. "
        "Не придумывай закон. Не отвечай общими фразами. "
        "Если пользователь спрашивает о конкретном лекарстве, оцени, похоже ли оно на обычный препарат для личного пользования, "
        "и отдельно укажи, что для наркотических и психотропных веществ правила строже. "
        "Если вопрос про товар для личного пользования, не требуй марку и модель без необходимости. "
        "Если вопрос про один товар, исходи из личного пользования. "
        "Отвечай кратко, уверенно и по делу."
    )
    base_rules_uz = (
        "Sen O‘zbekiston bojxona masalalari bo‘yicha jismoniy shaxslar uchun yordamchisan. "
        "Avval mahalliy qoidalarga tayangan holda javob ber. Jismoniy va yuridik shaxslarni aralashtirma. "
        "Qonunni to‘qib chiqarmagin. Umumiy va foydasiz javob yozma. "
        "Agar foydalanuvchi aniq bir dori nomini yozsa, uni odatiy shaxsiy foydalanish dorisiga o‘xshaydimi, shuni tushuntir "
        "va narkotik yoki psixotrop moddalar uchun qoidalar qat’iyroq ekanini ayt. "
        "Shaxsiy foydalanish uchun bitta tovar bo‘lsa, keraksiz ravishda marka va modelni so‘rama. "
        "Qisqa, aniq va ishonchli javob ber."
    )

    local_context = []
    if topic == "medicines":
        local_context.append(
            "Без меддокумента допускается до 10 разных лекарственных препаратов и не более 5 упаковок каждого. Изделий медицинского назначения — до 5 единиц. Для наркотических и психотропных веществ действуют отдельные правила."
            if lang == "ru" else
            "Tibbiy hujjatsiz 10 xil dori va har biridan 5 qadoqgacha ruxsat etiladi. Tibbiy buyumlar — 5 donagacha. Narkotik va psixotrop moddalar uchun alohida qoidalar mavjud."
        )
    elif topic == "tv":
        local_context.append(
            "Для телевизора через наземные пункты действует количественная норма 1 единица на 6 календарных месяцев. Лимит стоимости: аэропорт — до 1000 долларов США, авто/пешком — до 300 долларов США."
            if lang == "ru" else
            "Televizor uchun quruqlik punktlari orqali 6 kalendar oyda 1 dona me’yor qo‘llanadi. Qiymat limiti: aeroport — 1000 AQSH dollari, avto/piyoda — 300 AQSH dollari."
        )
    elif topic == "phone":
        local_context.append(
            "Через авиацию для телефонов действует норма до 2 единиц при каждом въезде."
            if lang == "ru" else
            "Aviatsiya orqali telefonlar uchun har kirishda 2 donagacha ruxsat etiladi."
        )
    elif topic == "notebook":
        local_context.append(
            "Через автодорожные, пешеходные, железнодорожные и речные пункты ноутбук обычно разрешён в количестве 1 единица на 6 календарных месяцев."
            if lang == "ru" else
            "Avtoyo‘l, piyoda, temir yo‘l va daryo punktlari orqali noutbuk odatda 6 kalendar oyda 1 dona miqdorida ruxsat etiladi."
        )
    elif topic == "currency":
        local_context.append(
            "Ввоз наличной валюты физлицами — без ограничений. Вывоз — до эквивалента 100 000 000 сумов."
            if lang == "ru" else
            "Naqd valyutani olib kirish cheklanmagan. Olib chiqish — 100 000 000 so‘m ekvivalentigacha."
        )
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        if ready_answer:
            ...
            ai_topic = physical_guess_topic_from_text(text)
            if topic:
                physical_set_state(c, topic, lang)
                c["mode"] = "physical_chat"
            if ai_topic:
                physical_set_ai_topic(c, ai_topic, text)
            await message.answer(
                ready_answer + "\n\n" + t(lang, "physical_free_specialist"),
                reply_markup=physical_ai_kb(lang) if topic else physical_ready_kb(lang)
            )
            return
    ...
    if c["mode"] == "physical_chat":
        ...
        # AI-уточнение после базового ответа для физлиц
