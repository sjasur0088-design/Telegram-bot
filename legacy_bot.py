...
from physical_pro_next_level import find_physical_answer_v2, get_physical_keyboard
...
def physical_rule_topic_from_text(text: str) -> str:
    q = normalize_text(text)
    if any(x in q for x in ["телевиз", "tv", "смарт тв", "smart tv", "телефон", "смартфон", "iphone", "айфон", "telefon", "phone", "noutbuk", "ноутбук", "laptop", "planshet", "планшет", "elektron", "электрон", "tehnik", "техник"]):
        return "electronics"
    if any(x in q for x in ["лекар", "dori", "tablet", "таблет", "анальгин", "ибупрофен", "парацетамол", "sitramon", "цитрамон", "prep", "препарат"]):
        return "medicine"
    if any(x in q for x in ["валют", "valyuta", "доллар", "usd", "евро", "eur", "налич", "cash", "naqd"]):
        return "currency"
    return ""
...
def physical_rule_base_answer(text: str, lang: str, topic: str = "") -> str:
    ...
    if topic == "electronics":
        return (
            "Texnika shaxsiy foydalanish uchun: 6 oyda 1 marta. Aeroport orqali limit — 1000 AQSH dollari. Quruqlik chegarasi orqali — 300 AQSH dollari. Limit oshsa — 30% yoki har 1 kg uchun 3 dollar to'lanadi."
            if lang == "uz" else
            "Техника для личного пользования: 1 раз в 6 месяцев. Через аэропорт лимит — 1000 долларов США. Через сухопутную границу — 300 долларов США. При превышении — 30% или 3 доллара за кг."
        )
    if topic == "medicine":
        return (
            "Shaxsiy foydalanish uchun odatda 10 xil dori va har biridan 5 qadoqqacha ruxsat etiladi. Narkotik, psixotrop moddalar va prekursorlar uchun alohida tartib bor va maxsus hujjatlar talab qilinishi mumkin."
            if lang == "uz" else
    ...
    if topic == "currency":
        return (
            "Naqd valyutani olib kirish odatda cheklanmagan. Olib chiqish bo'yicha esa amaldagi limit va deklaratsiya qoidalari qo'llanadi. Aniq summa bo'yicha yozing."
            if lang == "uz" else
            "Ввоз наличной валюты обычно не ограничен. По вывозу действуют лимит и правила декларирования. Напишите сумму, если нужен точный ответ."
        )
    return ""
...
def physical_should_use_rule_answer(text: str, c: Dict[str, Any]) -> str:
    ...
    q = normalize_text(text)
    if current in {"electronics", "medicine", "currency"}:
...
def physical_should_use_ai_followup_v2(text: str, c: Dict[str, Any]) -> bool:
    ...
    if topic not in {"electronics", "medicine", "currency"}:
        return False
    if looks_like_new_question(text):
        return False
    if is_legal_commercial_question(text):
        return False
    if physical_rule_topic_from_text(text):
        return False
    if len(q.split()) <= 12:
        return True
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        pro_answer = find_physical_answer_v2(uid, text)
        if pro_answer:
            c["mode"] = "physical_chat"
            await send_safe_message(message, pro_answer, reply_markup=physical_ai_kb(lang))
            return
