...
def load_physical_faq_pro() -> Dict[str, Dict[str, Any]]:
    candidate_paths = [
        os.path.join(BASE_DIR, "physical_faq.json"),
        os.path.join(BASE_DIR, "physical_faq_new_pro.json"),
    ]
    ...
    try:
        ...
        if not path:
            print("physical_faq json not found")
            return empty
        ...
        for lang in ("ru", "uz"):
            ...
            if isinstance(block, dict):
                prepared[lang]["global_rules"] = block.get("global_rules", {}) or {}
                prepared[lang]["faq_items"] = block.get("faq_items", []) or []

        print(f"Loaded physical faq json successfully: {os.path.basename(path)}")
        return prepared
    except Exception as e:
        print(f"Failed to load physical faq json: {e}")
        return empty
...
def physical_faq_pattern_match(text: str, lang: str) -> str:
    ...
    best_answer = ""
    best_score = 0
    faq_items = PHYSICAL_FAQ_PRO.get(lang, {}).get("faq_items", [])
    ...
    for item in faq_items:
        ...
        for p in item.get("patterns", []):
            ...
            if p_norm == q:
                ...
                p_words = [w for w in p_norm.split() if len(w) >= 3]
                for w in p_words:
                    if w in q:
                        score += 1
...
def physical_faq_global_hint(text: str, lang: str) -> str:
    q = normalize_text(text)
    rules = PHYSICAL_FAQ_PRO.get(lang, {}).get("global_rules", {}) or {}

    def fmt(rule_key: str) -> str:
        rule = rules.get(rule_key, {}) or {}
        answer = str(rule.get("answer", "")).strip()
        follow_up = str(rule.get("follow_up", "")).strip()
        if answer and follow_up:
            return answer + "\n\n" + follow_up
        return answer

    if any(x in q for x in ["аэропорт", "airport", "aeroport", "самолет", "самолёт", "avia", "авиа",
                            "авто", "машин", "автодорож", "пешком", "пешеход", "поезд", "жд", "temir yo'l",
                            "temir yol", "avto", "piyoda"]):
        txt = fmt("import_by_route")
        if txt:
            return txt

    if any(x in q for x in ["алког", "табак", "сигар", "духи", "ювелир", "atir", "tamaki", "alkogol"]):
        txt = fmt("special_import_limits")
        if txt:
            return txt

    if any(x in q for x in ["для продажи", "коммер", "партия", "одинаков", "tijorat", "partiya", "sotish"]):
        txt = fmt("commercial_use")
        if txt:
            return txt

    if any(x in q for x in ["превыш", "сверх", "платеж", "платёж", "сколько платить", "сколько заплачу",
                            "oshsa", "to'lov", "tolov"]):
        txt = fmt("single_customs_payment")
        if txt:
            return txt
...
def physical_answer(q: str, lang: str) -> str:
    ...
    if any(x in qn for x in ["валют", "valyuta", "usd", "доллар"]):
        return (
            "Ввоз наличной валюты физлицами в Узбекистан не ограничен. Вывоз наличной валюты разрешён до эквивалента 100 000 000 сумов. "
            "Если напишете валюту, сумму и это ввоз или вывоз, я отвечу точнее по вашей ситуации."
            if lang == "ru" else
            "Jismoniy shaxslar uchun naqd valyutani O‘zbekistonga olib kirish cheklanmagan. Naqd valyutani olib chiqish esa 100 000 000 so‘m ekvivalentigacha ruxsat etiladi. "
            "Valyuta turi, summa va bu olib kirishmi yoki olib chiqishmi — shuni yozsangiz, aniqroq aytaman."
        )
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        if not ready_answer:
            ready_answer = physical_faq_pattern_match(text, lang)
        if not ready_answer:
            ready_answer = physical_faq_global_hint(text, lang)
        if ready_answer:
    ...
    if c["mode"] == "physical_chat":
        ...
        # для физлиц не уводим в редирект, если вопрос покрывается глобальными правилами
        global_hint = physical_faq_global_hint(text, lang)
        if global_hint:
            physical_clear_state(c)
            await send_safe_message(
                message,
