...
def physical_redirect_text(lang: str) -> str:
    return (
        "Я отвечаю только по вопросам для физических лиц и личного пользования. "
        "По ставкам, ТН ВЭД, коммерческому импорту и расчётам перейдите в раздел для юридических лиц."
        if lang == "ru" else
        "Men faqat jismoniy shaxslar va shaxsiy foydalanish bo‘yicha savollarga javob beraman. "
        "Stavkalar, TN VED, tijorat importi va hisob-kitoblar bo‘yicha yuridik shaxslar bo‘limiga o‘ting."
    )
...
def physical_followup_answer(topic: str, detail: str, lang: str) -> str:
    item = detail.strip()
    q = normalize_text(item)

    if topic == "norm_exceeded":
        if any(x in q for x in ["iphone", "айфон", "telefon", "phone", "смартфон"]):
            return (
                f"Если речь о товаре «{item}» для личного пользования, обычно таможня смотрит количество, стоимость и признаки коммерческой партии. "
                "Один телефон для себя обычно рассматривается проще, чем несколько одинаковых устройств. "
                "Для точного ответа напишите, через какой пункт вы въезжаете и примерную стоимость товара."
                if lang == "ru" else
                f"Agar gap «{item}» haqida bo‘lsa va u shaxsiy foydalanish uchun olib kirilsa, bojxona odatda soni, qiymati va tijorat alomatlariga qaraydi. "
                "O‘zingiz uchun 1 dona telefon odatda bir nechta bir xil qurilmaga qaraganda osonroq ko‘riladi. "
                "Aniqroq javob uchun qaysi post orqali kirayotganingizni va taxminiy qiymatini yozing."
            )
        return (
            f"Agar norma «{item}» bo‘yicha oshsa, bojxona to‘lovlari qo‘llanishi mumkin. Aniq javob tovar turi, qiymati, vazni va olib kirish usuliga bog‘liq. "
            "Aniqroq javob uchun kirish yo‘li va taxminiy qiymatini yozing."
            if lang != "ru" else
            f"Если норма превышена по товару «{item}», могут применяться таможенные платежи. Точный ответ зависит от вида товара, стоимости, веса и способа ввоза. "
            "Для более точного ответа напишите способ въезда и примерную стоимость."
        )

    if topic == "medicine":
        if any(x in q for x in ["sitramon", "цитрамон", "paratsetamol", "парацетамол", "tablet", "таблет"]):
            return (
                f"Dori «{item}» odatda shaxsiy foydalanish uchun oz miqdorda olib kirilganda savol tarkib, miqdor va cheklovlarga bog‘liq bo‘ladi. "
                "Ba’zi dori vositalari uchun qo‘shimcha hujjat yoki shifokor tavsiyasi kerak bo‘lishi mumkin. "
                "Nechta qadoq olib kirayotganingizni yozing."
                if lang != "ru" else
                f"По лекарству «{item}» для личного пользования обычно смотрят состав, количество и возможные ограничения. "
                "По отдельным препаратам могут понадобиться дополнительные документы или подтверждение назначения врача. "
                "Напишите, сколько упаковок вы везёте."
            )
        return (
            f"Dori «{item}» bo‘yicha tarkib, miqdor va maqsad muhim. Agar shaxsiy foydalanish uchun bo‘lsa, odatda miqdor asosiy rol o‘ynaydi. "
            "Nechta qadoq olib kirayotganingizni yozing."
            if lang != "ru" else
            f"По лекарству «{item}» важны состав, количество и цель ввоза. Если для личного пользования, обычно ключевое значение имеет объём. "
            "Напишите, сколько упаковок вы везёте."
        )

    if topic == "phones":
        return (
            f"Если речь о «{item}», для физлица важны количество, личное пользование и IMEI-регистрация. "
            "Один телефон для себя обычно проще, чем несколько одинаковых устройств. "
            "Напишите, сколько именно телефонов и как вы въезжаете."
            if lang == "ru" else
            f"Agar gap «{item}» haqida bo‘lsa, jismoniy shaxs uchun soni, shaxsiy foydalanish va IMEI ro‘yxatdan o‘tkazish muhim. "
            "O‘zingiz uchun 1 dona telefon odatda bir nechta bir xil qurilmaga qaraganda osonroq ko‘riladi. "
            "Nechta telefon olib kirayotganingizni va qanday kirayotganingizni yozing."
        )

    if topic == "currency":
        return (
            f"По валюте для точного ответа нужна сумма и вид валюты. Если речь о «{item}», напишите точную сумму."
            if lang == "ru" else
            f"Valyuta bo‘yicha aniq javob uchun summa va valyuta turi kerak. Agar gap «{item}» haqida bo‘lsa, aniq summani yozing."
        )

    return (
        f"Уточните, что именно вы хотите ввезти: «{item}», количество и как вы пересекаете границу."
        if lang == "ru" else
        f"Aniqroq yozing: «{item}» nima, qancha miqdorda va chegarani qanday kesib o‘tyapsiz."
    )
...
def ai_followup_answer(state: Dict[str, Any], lang: str) -> str:
    ...
    food_words = ["pomidor", "томат", "tomat", "olma", "apple", "яблок", "meva", "fruit", "sabzavot", "овощ"]
    is_food = any(w in normalize_text(product) for w in food_words)
...
async def router(message: types.Message):
    ...
    if c["mode"] == "legal_chat":
        ...
        if client:
            try:
                system_prompt = (
                    "Ты — таможенный помощник по Узбекистану. "
                    "Отвечай только по теме таможни, импорта, экспорта, документов, сертификации, TN VED и ставок. "
                    "Пиши простым понятным языком, но профессионально. "
                    "Если данных мало, сначала дай базовый ответ и задай один точный уточняющий вопрос. "
                    "Если пользователь отвечает коротко одним словом, воспринимай это как уточнение предыдущего таможенного вопроса."
                    if lang == "ru" else
                    "Sen O‘zbekiston bo‘yicha bojxona yordamchisisan. "
                    "Faqat bojxona, import, eksport, hujjatlar, sertifikatlash, TN VED va stavkalar mavzusida javob ber. "
                    "Oddiy va tushunarli, lekin professional tilda yoz. "
                    "Ma’lumot yetarli bo‘lmasa, avval qisqa asosiy javob ber va bitta aniq savol bilan aniqlashtir. "
    ...
    if c["mode"] == "physical_ready":
        ...
        if c.get("physical_ai_state", {}).get("topic"):
            ...
            await message.answer(
                physical_followup_answer(state.get("topic", ""), text.strip(), lang) + "\n\n" + t(lang, "physical_free_specialist"),
                reply_markup=physical_ai_kb(lang)
            )
            return
    ...
    if c["mode"] == "physical_chat":
        ...
        # If user sent a short clarification like "Sitramon" or "iPhone 17 Pro 1 dona"
        if state.get("topic"):
