...
def looks_like_customs_question(text: str) -> bool:
    ...
    keywords = [
        ...
        # physical-person questions that users ask without the word "customs"
        "телевизор", "tv", "smart tv", "смарт тв", "televizor",
        "телефон", "iphone", "айфон", "смартфон", "phone", "telefon",
        "лекар", "dori", "tablet", "таблет", "цитрамон", "парацетамол", "анальгин", "ибупрофен",
        "ноутбук", "laptop", "noutbuk", "компьютер",
        "валют", "valyuta", "доллар", "usd", "евро", "eur",
        "аэропорт", "airport", "aeroport", "границ", "chegara", "пост",
        "для себя", "личного пользования", "shaxsiy foydalanish",
        "россия", "russia", "дубай", "dubay", "казахстан", "qozog'iston", "кыргызстан"
    ]
...
def physical_pro_pattern_answer(text: str, lang: str) -> str:
    ...
    if topic == "tv":
        return (
            "Можно 1 телевизор для личного пользования. Через сухопутную границу, ж/д и речные пункты — 1 шт. в 6 месяцев. Лимит по стоимости: аэропорт — 1000$, граница — 300$. Сверх лимита — 30% или минимум 3$/кг."
            if lang == "ru" else
            "Shaxsiy foydalanish uchun 1 dona televizor mumkin. Quruqlik, temir yo‘l va daryo punktlari orqali — 6 oyda 1 dona. Qiymat limiti: aeroport — 1000$, chegara — 300$. Limitdan oshsa — 30% yoki kamida 3$/kg."
        )
    if topic == "notebook":
        return (
            "Ноутбук для личного пользования обычно можно ввозить 1 шт. в 6 месяцев через сухопутную границу, ж/д и речные пункты. Если 2 и более, таможня может посчитать это превышением нормы или не личным пользованием."
            if lang == "ru" else
            "Noutbukni shaxsiy foydalanish uchun odatda quruqlik, temir yo‘l va daryo punktlari orqali 6 oyda 1 dona olib kirish mumkin. 2 dona va undan ko‘p bo‘lsa, bu me’yor oshishi yoki tijorat deb baholanishi mumkin."
        )
    if topic == "phones":
        return (
            "Через аэропорт обычно можно до 2 телефонов за один въезд. Если телефонов больше или это похоже на партию, таможня может признать это не личным пользованием."
            if lang == "ru" else
            "Aeroport orqali odatda bir kirishda 2 tagacha telefon mumkin. Ko‘proq bo‘lsa yoki partiyaga o‘xshasa, bojxona buni shaxsiy foydalanish deb hisoblamasligi mumkin."
        )
    if topic == "currency":
        return (
...
def physical_needs_ai_followup(text: str, c: Dict[str, Any], lang: str) -> bool:
    ...
    short_followup = 1 <= len(words) <= 8
    has_digits = any(ch.isdigit() for ch in text)
    latin_or_model = bool(re.search(r"[A-Za-zА-Яа-я]+[-_/]?[0-9]{1,4}|[0-9]{2,3}\s?(?:дюйм|inch|in|см|size|размер)", text, re.I))
    ...
    if topic == "medicine":
        if 1 <= len(words) <= 6 and not any(w in q for w in generic):
            return True
        if has_digits or latin_or_model:
            return True
...
def physical_topic_rule_text(topic: str, lang: str) -> str:
    ...
    mapping = {
        ...
        'phones': (
            'Телефон для личного пользования обычно можно. Важны способ въезда, цена, вес и количество. Напишите: через аэропорт или границу, сколько стоит телефон и сколько штук везёте.'
            if lang == 'ru' else
            'Telefonni shaxsiy foydalanish uchun odatda olib kirish mumkin. Kirish usuli, narxi, vazni va soni muhim. Yozing: aeroportmi yoki chegara, telefon narxi qancha va nechta olib kelyapsiz.'
        ),
        'notebook': (
            'Ноутбук для личного пользования обычно можно ввозить 1 шт. в 6 месяцев через сухопутную границу, ж/д и речные пункты. Если 2 и более, таможня может посчитать это превышением нормы или не личным пользованием.'
            if lang == 'ru' else
...
def physical_safe_followup_answer(text: str, c: Dict[str, Any], lang: str) -> str:
    topic = physical_get_pro_topic(c)
    if not topic:
        return ''
    base = physical_topic_rule_text(topic, lang) or physical_pro_pattern_answer(topic, lang)
    q = normalize_text(text)
    if topic == 'tv':
        if lang == 'ru':
            return base + ' Если речь об одном телевизоре для себя, обычно вопрос решается по цене и способу ввоза. Напишите цену и как везёте: аэропорт или граница.'
        return base + ' Agar gap o‘zingiz uchun 1 dona televizor haqida bo‘lsa, odatda masala narx va olib kirish usuliga bog‘liq. Narxini va qanday olib kirayotganingizni yozing: aeroport yoki chegara.'
    if topic == 'phones':
        if lang == 'ru':
            return base + ' Если это iPhone или другой смартфон для себя, дальше нужно смотреть цену, количество и как везёте.'
        return base + ' Agar bu iPhone yoki boshqa smartfon bo‘lsa, keyin narxi, soni va qanday olib kirayotganingiz muhim.'
    if topic == 'medicine':
        if lang == 'ru':
            return base + ' Если это обычное лекарство для личного пользования, обычно проблем меньше. По конкретному препарату важно название и количество.'
        return base + ' Agar bu shaxsiy foydalanish uchun oddiy dori bo‘lsa, odatda muammo kamroq bo‘ladi. Aniq preparat bo‘yicha nomi va miqdori muhim.'
    if topic == 'notebook':
        if lang == 'ru':
            return base + ' Если это один ноутбук для себя, обычно смотрят количество и признаки личного пользования.'
        return base + ' Agar bu o‘zingiz uchun 1 dona noutbuk bo‘lsa, odatda soni va shaxsiy foydalanish alomatlari ko‘riladi.'
    return ''
...
def physical_pro_pattern_answer(text: str, lang: str) -> str:
    ...
    if topic == "phones":
        ...
        if direction == "import" and transport:
            ...
            if amount and limit and amount > limit:
                ...
                return (
                    f"Это похоже на ввоз телефона для личного пользования через {('аэропорт' if transport=='air' else 'границу')}. Цена: {amount:.2f}$. Лимит: {limit}$. Превышение: {excess:.2f}$. Сверх лимита — 30% или минимум 3$/кг."
                    if lang == "ru" else
                    f"Bu shaxsiy foydalanish uchun telefon olib kirishga o‘xshaydi. Narxi: {amount:.2f}$. Limit: {limit}$. Oshgan qismi: {excess:.2f}$. Limitdan oshsa — 30% yoki kamida 3$/kg."
                )
        return (
            "Телефон для личного пользования обычно можно. Важны способ въезда, цена, вес и количество. Напишите: через аэропорт или границу, сколько стоит телефон и сколько штук везёте."
            if lang == "ru" else
            "Telefonni shaxsiy foydalanish uchun odatda olib kirish mumkin. Kirish usuli, narxi, vazni va soni muhim. Yozing: aeroportmi yoki chegara, telefon narxi va nechta dona."
        )
    ...
    if topic == "medicine":
        return (
            "Для личного пользования лекарства обычно можно. Без меддокумента — до 10 разных лекарств и не более 5 упаковок каждого. Для наркотических, психотропных препаратов и прекурсоров действует отдельный порядок. Напишите название лекарства и количество — скажу точнее."
            if lang == "ru" else
            "Shaxsiy foydalanish uchun dorilar odatda mumkin. Tibbiy hujjatsiz — 10 xil dori va har biridan 5 qadoqqacha. Narkotik, psixotrop preparatlar va prekursorlar uchun alohida tartib bor. Dori nomi va sonini yozing — aniqroq aytaman."
        )
...
def build_physical_ai_prompt(user_text: str, c: Dict[str, Any], lang: str) -> str:
    ...
    if hint and hint not in local:
        local.append(hint)
    topic_rule = physical_topic_rule_text(topic, lang)
    if topic_rule and topic_rule not in local:
        local.insert(0, topic_rule)
    if lang == "uz":
        base = (
            ...
            "Faqat Local context ichidagi raqamlar, limitlar, foizlar va summalardan foydalan. Yangi raqam, eski norma yoki xotiradan olingan limitni uydirma qilma. Agar Local contextda aniq raqam bo‘lmasa, raqam yozma — narx va olib kirish usulini so‘ra. "
            "Javobni juda qisqa, aniq va foydali yoz."
        )
    else:
        base = (
            ...
            "Используй только цифры, лимиты, проценты и суммы из блока Local context. Запрещено придумывать новые числа, вспоминать старые нормы или писать суммы по памяти. Если в Local context нет точной цифры — не называй цифру, а попроси цену и способ ввоза. "
            "Отвечай очень коротко: 2-4 предложения. Сначала дай прямое правило из Local context. Потом коротко применяй его к примеру пользователя. В конце, если есть неопределенность, добавь одну фразу про специалиста. Не уходи в длинные объяснения."
        )
...
def build_physical_doc_first_answer(uid: int, text: str, lang: str):
    c = ctx(uid)
    local_answer = physical_pro_pattern_answer(text, lang) or physical_pro_global_hint(text, lang)

    if not PHYSICAL_ROUTER or not PHYSICAL_ENGINE:
        fallback = local_answer or physical_answer_legacy(text, lang)
    ...
    topic = physical_pro_guess_topic(text)
    if local_answer and topic in {"medicine", "tv", "phones", "notebook", "currency", "auto", "import"}:
        answer = local_answer
    elif not answer:
        answer = local_answer or physical_answer_legacy(text, lang)
...
def physical_doc_should_use_ai(route: Dict[str, Any]) -> bool:
    ...
    if score <= 0:
        return True
    if category in {"electronics", "medicine", "forbidden"} and not item:
        return True
    if intent in {"unknown", "clarify"}:
        return True
    return False
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_chat":
        ...
        prev_topic = physical_get_pro_topic(c)

        if prev_topic and physical_needs_ai_followup(text, c, lang):
            safe_followup = physical_safe_followup_answer(text, c, lang)
            if safe_followup:
                physical_clear_state(c)
                physical_set_pro_topic(c, prev_topic, text)
                await send_safe_message(
                    message,
                    safe_followup + "\n\n" + t(lang, "physical_free_specialist"),
                    reply_markup=physical_ai_kb(lang)
        ...
        # full AI fallback: не отсекаем нетипичные формулировки сообщением only_customs
        ai_text = physical_ai_answer(text, c, lang)
        if ai_text:
            physical_clear_state(c)
            physical_set_pro_topic(c, physical_pro_guess_topic(text), text)
