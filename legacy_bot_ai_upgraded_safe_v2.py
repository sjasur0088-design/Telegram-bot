...
def ctx(uid: int) -> Dict[str, Any]:
    if uid not in USER_CTX:
        USER_CTX[uid] = {"lang":"ru","role":None,"mode":None,"category":None,"group":None,"pending_form":None,"form_data":{},"chat_history":[],"physical_ai_state":{}}
    return USER_CTX[uid]
...
def ai_hint(query: str, items: List[Dict[str, Any]], lang: str) -> str:
    ...
    try:
        ...
        system = (
            "Ты помощник по таможне Узбекистана. Отвечай кратко, точно и без выдуманных кодов. Используй только переданные данные. Если данных мало — прямо скажи это."
            if lang == "ru" else
            "Sen O‘zbekiston bojxona yordamchisisan. Qisqa va aniq javob ber. Faqat berilgan ma’lumotlardan foydalan. Ma’lumot yetarli bo‘lmasa, buni ochiq ayt."
        )
        user_prompt = (
            f"Запрос пользователя: {query}\nДоступные данные: {json.dumps(data, ensure_ascii=False)}"
            if lang == "ru" else
            f"Foydalanuvchi so‘rovi: {query}\nMavjud ma’lumotlar: {json.dumps(data, ensure_ascii=False)}"
        )
...
SMART_CUSTOMS_KB = {
    "ru": {
        "import_limits": (
            "Для физлиц действуют лимиты беспошлинного ввоза в зависимости от способа въезда: через международные аэропорты — до 2000 долларов США, через железнодорожные и речные пункты — до 1000 долларов США, через автодорожные и пешеходные пункты — до 300 долларов США. При превышении применяется единый таможенный платеж 30% от таможенной стоимости, но не менее 3 долларов США за кг."
        ),
        "special_import_limits": (
            "Отдельные нормы беспошлинного ввоза: алкоголь — до 2 литров, табачные изделия — до 10 пачек, духи и туалетная вода — до 3 единиц, ювелирные изделия — до 65 граммов."
        ),
        "goods_quantity_land": (
            "Через автодорожные, пешеходные, железнодорожные и речные пункты пропуска действуют количественные нормы. Например, холодильник, телевизор, стиральная машина, ноутбук, планшет, компьютерная техника — обычно 1 единица в установленный период. Для прочих товаров — по 2 кг каждого наименования, но не более 10 кг общего веса."
        ),
        "phone_rule_air": (
            "Для авиапересечения телефонные аппараты, в том числе мобильные телефоны и устройства с модулем мобильной связи, разрешаются до 2 единиц при каждом въезде."
        ),
        "export_limits": (
            "При выезде из Узбекистана физлицу разрешается вывозить товары общей стоимостью не более 5000 долларов США без подачи таможенной декларации. Есть отдельные нормы вывоза: рис — до 3 кг, хлебобулочные изделия — до 5 кг, мясо и мясные субпродукты — до 2 кг, сахар — до 2 кг, растительные масла — до 2 кг, плодоовощная продукция — до 40 кг."
        ),
        "currency": (
            "Ввоз наличной валюты физлицами в Узбекистан осуществляется без ограничений. Вывоз наличной валюты физлицами за пределы Узбекистана допускается в сумме, не превышающей эквивалент 100 000 000 сумов. Сверх этой суммы действует отдельный порядок."
        ),
        "medicines": (
            "Для личного пользования разрешается ввоз и вывоз лекарственных средств и медизделий. Без меддокумента допускается до 10 лекарственных препаратов разного наименования и не более 5 упаковок каждого, а медизделий — не более 5 единиц. Лекарства должны быть в заводской упаковке. Для наркотических и психотропных веществ действуют отдельные строгие правила и декларирование."
        ),
        "pyro": (
            "Пиротехнические средства относятся к чувствительной категории. По ним нужны осторожные ответы с указанием ограничений и запретов."
        ),
        "vehicles": (
            "По транспортным средствам для физлиц действует отдельный порядок. Временный ввоз транспортного средства для некоммерческих целей обычно не должен превышать 90 календарных дней в течение календарного года, если иное не оформлено в установленном порядке."
        ),
        "prohibited": (
            "Существуют запрещенные и ограниченные к ввозу/вывозу категории: наркотические и психотропные вещества, взрывоопасные вещества, пиротехника, отдельные виды оружия, культурные ценности, озоноразрушающие вещества, некоторые радиоэлектронные средства и другие товары, для которых требуются разрешения."
        ),
        "commercial_signs": (
            "Если количество однородных товаров превышает обычную потребность физлица и членов его семьи, таможня может рассматривать такие товары как ввозимые для предпринимательской деятельности, если лицо не докажет обратное."
        ),
    },
    "uz": {
        "import_limits": (
            "Jismoniy shaxslar uchun bojsiz olib kirish limiti kirish usuliga bog‘liq: xalqaro aeroportlar orqali — 2000 AQSH dollarigacha, temir yo‘l va daryo punktlari orqali — 1000 AQSH dollarigacha, avtoyo‘l va piyoda punktlari orqali — 300 AQSH dollarigacha. Me’yor oshsa, yagona bojxona to‘lovi qo‘llanadi: bojxona qiymatining 30 foizi, lekin kamida har kg uchun 3 AQSH dollari."
        ),
        "special_import_limits": (
            "Alohida bojsiz me’yorlar: alkogol — 2 litrgacha, tamaki mahsulotlari — 10 pachkagacha, atir va tualet suvi — 3 birlikkacha, zargarlik buyumlari — 65 grammgacha."
        ),
        "goods_quantity_land": (
            "Avtoyo‘l, piyoda, temir yo‘l va daryo punktlari orqali ayrim tovarlar uchun miqdoriy me’yorlar mavjud. Masalan, muzlatgich, televizor, kir yuvish mashinasi, noutbuk, planshet, kompyuter texnikasi — odatda belgilangan davrda 1 dona. Boshqa tovarlar uchun — har bir nomidan 2 kg, lekin umumiy og‘irligi 10 kg dan oshmasligi kerak."
        ),
        "phone_rule_air": (
            "Aviatsiya orqali kirishda telefon apparatlari, shu jumladan mobil telefonlar va mobil aloqa moduliga ega qurilmalar, har bir kirishda 2 donagacha."
        ),
        "export_limits": (
            "O‘zbekistondan chiqishda jismoniy shaxs bojxona deklaratsiyasisiz umumiy qiymati 5000 AQSH dollaridan oshmaydigan tovarlarni olib chiqishi mumkin. Alohida me’yorlar ham bor: guruch — 3 kg gacha, non mahsulotlari — 5 kg gacha, go‘sht mahsulotlari — 2 kg gacha, shakar — 2 kg gacha, o‘simlik yog‘i — 2 kg gacha, meva-sabzavot va o‘xshash mahsulotlar — 40 kg gacha."
        ),
        "currency": (
            "Jismoniy shaxslar tomonidan naqd valyutani O‘zbekistonga olib kirish cheklanmagan. Naqd valyutani O‘zbekistondan olib chiqish esa 100 000 000 so‘m ekvivalentidan oshmagan miqdorda ruxsat etiladi. Undan yuqori summa uchun alohida tartib mavjud."
        ),
        "medicines": (
            "Shaxsiy foydalanish uchun dori vositalari va tibbiy buyumlarni olib kirish va olib chiqishga ruxsat etiladi. Tibbiy hujjatsiz 10 xil nomdagi dori vositasigacha va har biridan 5 qadoqdan oshmagan miqdorda, tibbiy buyumlar esa 5 donagacha olib o‘tilishi mumkin. Dorilar ishlab chiqaruvchi qadoqlarida bo‘lishi kerak. Narkotik va psixotrop moddalar uchun alohida qat’iy qoidalar mavjud."
        ),
        "pyro": (
            "Pirotexnika sezgir toifa hisoblanadi. Bunday tovarlar bo‘yicha cheklov va taqiqlarni darhol aytish kerak."
        ),
        "vehicles": (
            "Transport vositalari bo‘yicha jismoniy shaxslar uchun alohida tartib mavjud. Vaqtincha olib kirilgan transport vositasining umumiy muddati odatda bir kalendar yil davomida 90 kalendar kundan oshmasligi kerak."
        ),
        "prohibited": (
            "Taqiqlangan yoki cheklangan toifalar mavjud: narkotik va psixotrop moddalar, portlovchi moddalar, pirotexnika, ayrim qurol va patronlar, madaniy boyliklar, ozonni yemiruvchi moddalar va boshqa ruxsat talab qiladigan tovarlar."
        ),
        "commercial_signs": (
            "Agar bir xil tovarlar soni jismoniy shaxs va uning oila a’zolari ehtiyojidan ortiq bo‘lsa, bojxona buni tadbirkorlik faoliyati uchun olib kirilayotgan tovar deb baholashi mumkin, agar buning aksi isbotlanmasa."
        ),
    }
}


def get_chat_memory(uid: int) -> List[Dict[str, str]]:
    c = ctx(uid)
    history = c.setdefault("chat_history", [])
...
def push_chat_memory(uid: int, role: str, content: str) -> None:
    history = get_chat_memory(uid)
    history.append({"role": role, "content": (content or "")[:4000]})
    if len(history) > CHAT_MEMORY_LIMIT:
        del history[:-CHAT_MEMORY_LIMIT]


def clear_chat_memory(uid: int) -> None:
    ctx(uid)["chat_history"] = []


def detect_customs_topics(text: str) -> List[str]:
    q = (text or "").lower()
    topics = []
    if any(x in q for x in ["ввоз", "ввез", "въез", "привез", "провез", "олиб кир", "import", "импорт"]):
        topics.append("import_limits")
    if any(x in q for x in ["вывоз", "вывез", "выез", "олиб чиқ", "olib chiq", "export", "экспорт"]):
        topics.append("export_limits")
    if any(x in q for x in ["валют", "доллар", "евро", "сум", "налич", "cash", "currency", "pul", "valyut"]):
        topics.append("currency")
    if any(x in q for x in ["лекар", "таблет", "капсул", "ампул", "укол", "инсулин", "медицин", "dori", "tablet", "kapsul", "ampula", "tibbiy"]):
        topics.append("medicines")
    if any(x in q for x in ["пиротех", "салют", "фейер", "петард", "pyro", "pirotex", "salyut"]):
        topics.append("pyro")
    if any(x in q for x in ["машин", "авто", "автомоб", "транспорт", "car", "vehicle", "mashina", "avto"]):
        topics.append("vehicles")
    if any(x in q for x in ["нельзя", "запрещ", "разрешен", "разрешение", "огранич", "taqiql", "ruxsat", "chekl"]):
        topics.append("prohibited")
    if any(x in q for x in ["сколько", "количество", "норма", "лимит", "qancha", "nechta", "miqdor", "limit", "meyor"]):
        topics.append("goods_quantity_land")
    if any(x in q for x in ["телефон", "айфон", "iphone", "смартфон", "mobil", "telefon"]):
        topics.extend(["phone_rule_air", "commercial_signs"])
    if any(x in q for x in ["для продажи", "коммер", "перепродаж", "biznes", "tijorat", "sotish"]):
        topics.append("commercial_signs")
...
def build_customs_context(user_text: str, lang: str) -> str:
    kb = SMART_CUSTOMS_KB["uz"] if lang == "uz" else SMART_CUSTOMS_KB["ru"]
    topics = detect_customs_topics(user_text)
    if not topics:
        topics = ["import_limits", "special_import_limits", "commercial_signs"]
    parts = []
    for key in topics[:5]:
        value = kb.get(key)
        if value:
            parts.append(f"[{key}] {value}")
    if "import_limits" in topics and "special_import_limits" not in topics:
        parts.append(f"[special_import_limits] {kb.get('special_import_limits', '')}")
    return "\n".join(p for p in parts if p.strip())


def faq_quick_match(user_text: str, lang: str) -> str:
    base = PHYSICAL_FAQ.get(lang, {})
...
def build_customs_system_prompt(lang: str, role: str) -> str:
    if lang == "uz":
        return f"""
Sen O‘zbekiston bojxona masalalari bo‘yicha aqlli yordamchisan.
Hozir foydalanuvchi rejimi: {role}.

QOIDALAR:
1. Faqat bojxona, import, eksport, jismoniy shaxslar olib kirishi/olib chiqishi, hujjatlar, cheklovlar va ruxsatlar bo‘yicha javob ber.
2. Birinchi navbatda berilgan mahalliy normativ kontekstga tayangan holda yoz.
3. Bilmasang to‘qib chiqarmagin. "Aniqlashtirish kerak" deb ayt.
4. Jismoniy shaxs va yuridik shaxs masalalarini aralashtirma.
5. Suhbatni insoniy tarzda davom ettir va kerak bo‘lsa 1 ta aniqlashtiruvchi savol ber.
6. Juda uzun yozma. Qisqa, ishonchli va aniq yoz.
7. Agar tovar miqdori ko‘p bo‘lsa, shaxsiy foydalanish yoki tijorat maqsadi bo‘lishi mumkinligini ayt.
8. Agar tovar taqiqlangan yoki ruxsat talab qilsa, darhol ogohlantir.

FORMAT:
- 1-qator: asosiy xulosa
- keyin 1-2 qisqa tushuntirish
- oxirida kerak bo‘lsa 1 ta aniqlashtiruvchi savol
"""
    return f"""
Ты — умный помощник по таможенным вопросам Узбекистана.
Текущий режим пользователя: {role}.

ПРАВИЛА:
1. Отвечай только по таможне, импорту, экспорту, ввозу/вывозу физлицами, документам, ограничениям и разрешениям.
2. В первую очередь опирайся на переданный локальный нормативный контекст.
3. Ничего не выдумывай. Если данных мало — прямо скажи, что нужно уточнить.
4. Не смешивай вопросы физлиц и юрлиц.
5. Веди разговор по-человечески: при необходимости задай 1 уточняющий вопрос.
6. Пиши уверенно, кратко и понятно.
7. Если количество товаров выглядит большим, укажи, что таможня может оценить это как коммерческую цель.
8. Если товар запрещённый или требующий разрешения — сразу предупреди об этом.

ФОРМАТ:
- первая строка: главный вывод
- затем 1–2 коротких пояснения
- в конце при необходимости 1 уточняющий вопрос
"""


def build_customs_messages(uid: int, user_text: str, lang: str, role: str) -> List[Dict[str, str]]:
    messages = [{"role": "system", "content": build_customs_system_prompt(lang, role)}]
    context = build_customs_context(user_text, lang)
    if context:
        messages.append({
            "role": "system",
            "content": f"Локальная нормативная база:\n{context}" if lang == "ru" else f"Mahalliy normativ baza:\n{context}"
        })
    quick = faq_quick_match(user_text, lang)
    if quick:
        messages.append({
            "role": "system",
            "content": f"Быстрый FAQ-слой:\n{quick}" if lang == "ru" else f"Tezkor FAQ qatlami:\n{quick}"
        })
    history = get_chat_memory(uid)
    if history:
...
def smart_customs_fallback(user_text: str, lang: str) -> str:
    topics = detect_customs_topics(user_text)
    if lang == "uz":
        if "currency" in topics:
            return "Naqd valyutani olib kirish cheklanmagan. Olib chiqish esa 100 000 000 so‘m ekvivalentigacha ruxsat etiladi. Qaysi valyuta va qancha summa haqida gap ketayotganini yozing."
        if "medicines" in topics:
            return "Dorilarni shaxsiy foydalanish uchun olib kirish yoki olib chiqish mumkin, lekin miqdor va tarkib muhim. Dori nomi, soni va retsept bor-yo‘qligini yozing."
        if "vehicles" in topics:
            return "Avto bo‘yicha alohida tartib mavjud. Vaqtincha olib kirishmi, olib chiqishmi yoki yangi mashina haqida savolmi — shuni aniqlashtiring."
        if "pyro" in topics:
            return "Pirotexnika bo‘yicha cheklovlar jiddiy. Aynan qaysi mahsulot ekanini yozing, men aniqroq yo‘naltiraman."
        return "Savolingiz bojxona qoidalariga taalluqli bo‘lsa, men yordam beraman. Tovar nomi, miqdori va qaysi yo‘l orqali olib kirish yoki olib chiqishni yozing."
    if "currency" in topics:
        return "Ввоз наличной валюты физлицами не ограничен. Вывоз — до эквивалента 100 000 000 сумов. Напишите, какая валюта и какая сумма у вас."
    if "medicines" in topics:
        return "Лекарства для личного пользования ввозить и вывозить можно, но важны название, количество и наличие меддокумента. Напишите препарат и количество."
    if "vehicles" in topics:
        return "По автомобилям действует отдельный порядок. Уточните: это временный ввоз, временный вывоз или покупка автомобиля."
    if "pyro" in topics:
        return "По пиротехнике действуют серьёзные ограничения. Напишите точное название товара, и я сориентирую точнее."
    return "Я помогу по таможенным правилам. Напишите товар, количество и как именно вы пересекаете границу — аэропорт, авто, поезд или пешком."


def ask_openai_chat(messages: List[Dict[str, str]]) -> str:
    if not client:
        return ""
    try:
        resp = client.chat.completions.create(
        ...
        logging.exception("ask_openai_chat error: %s", e)
        return ""


def smart_customs_answer(uid: int, user_text: str, lang: str, role: str = "physical") -> str:
    messages = build_customs_messages(uid, user_text, lang, role)
    answer = ask_openai_chat(messages)
    if not answer:
        answer = smart_customs_fallback(user_text, lang)
    push_chat_memory(uid, "user", user_text)
    push_chat_memory(uid, "assistant", answer)
    return answer
...
def physical_answer(q: str, lang: str, uid: int = None) -> str:
    qn = normalize_text(q)
    ...
    if any(x in qn for x in ["телефон", "iphone", "айфон", "смартфон", "phone", "telefon"]):
        if any(x in qn for x in ["дубай", "dubay", "аэропорт", "airport", "aeroport"]):
            return (
                "По телефонам для физлица важны количество, стоимость, цель ввоза и личное пользование. При авиаввозе телефоны и устройства с модулем мобильной связи обычно смотрят по норме до 2 единиц на каждый въезд. Несколько одинаковых новых устройств могут вызвать вопрос о коммерческой цели. Напишите, сколько именно телефонов вы везёте и их примерную стоимость."
                if lang == "ru" else
                "Telefonlar bo‘yicha jismoniy shaxs uchun soni, qiymati, olib kirish maqsadi va shaxsiy foydalanish muhim. Aviatsiya orqali kirishda telefon va mobil aloqa moduliga ega qurilmalar odatda har bir kirishda 2 donagacha ko‘riladi. Bir xil yangi qurilmalar ko‘p bo‘lsa, tijorat maqsadi haqida savol tug‘ilishi mumkin. Nechta telefon olib kirayotganingizni va taxminiy qiymatini yozing."
            )
        return (
            "По телефонам важны количество, стоимость, личное пользование и IMEI-регистрация. Если устройств несколько, таможня может дополнительно оценить цель ввоза. Напишите, сколько телефонов вы везёте и как пересекаете границу."
            if lang == "ru" else
            "Telefon bo‘yicha soni, qiymati, shaxsiy foydalanish va IMEI ro‘yxatdan o‘tkazish muhim. Qurilmalar bir nechta bo‘lsa, bojxona olib kirish maqsadini qo‘shimcha baholashi mumkin. Nechta telefon olib kirayotganingizni va qaysi yo‘l bilan kirayotganingizni yozing."
        )

    if uid is not None:
        return smart_customs_answer(uid, q, lang, role="physical")
    return smart_customs_fallback(q, lang)
...
def is_physical_rate_question(text: str) -> bool:
    q = normalize_text(text)
    keys = [
        "тн вэд", "tn ved", "tnved", "код тн вэд", "код товара", "ставка", "пошлина",
        "ндс", "акциз", "hs code", "код", "duty", "vat", "excise",
        "tnved kodi", "stavka", "boj", "aksiz", "nds", "kod",
        "какая ставка", "какой код", "подберите код", "aniq kod", "qaysi kod"
    ]
...
async def start_cmd(message: types.Message):
    USER_CTX[message.from_user.id] = {"lang":"ru","role":None,"mode":"choose_lang","category":None,"group":None,"pending_form":None,"form_data":{},"chat_history":[],"physical_ai_state":{}}
    ...
    if c["mode"] == "legal_chat":
        ...
        if client:
            try:
                ...
                await message.answer((content or t(lang, 'nothing_found')) + "\n\n" + t(lang, "free_specialist"), reply_markup=legal_ai_kb(lang))
            except Exception:
    ...
    if c["mode"] == "physical_chat":
        ...
        if not looks_like_customs_question(text):
            if looks_like_physical_customs_question(text):
                physical_clear_state(c)
                await message.answer(physical_answer(text, lang, uid), reply_markup=physical_ai_kb(lang))
                return
            await message.answer(
