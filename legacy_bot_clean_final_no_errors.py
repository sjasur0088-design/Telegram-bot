...
# ===== FINAL PHYSICAL PATCH HELPERS =====
def physical_mem_get(c: Dict[str, Any]) -> Dict[str, Any]:
    return c.setdefault("physical_short_memory", {"text": "", "topic": ""})

def physical_mem_set(c: Dict[str, Any], text: str, topic: str = "") -> None:
    c["physical_short_memory"] = {"text": (text or "")[:500], "topic": topic or ""}

def physical_ready_answer_from_big_faq(user_text: str, lang: str) -> str:
    q = normalize_text(user_text)
    block = PHYSICAL_FAQ_PRO.get(lang, {}) if isinstance(PHYSICAL_FAQ_PRO, dict) else {}
    items = block.get("faq_items", []) if isinstance(block, dict) else []
    best_answer = ""
    best_score = 0
    for item in items:
        patterns = item.get("patterns", []) or []
        for p in patterns:
            pp = normalize_text(str(p))
            if not pp:
                continue
            score = 0
            if q == pp or button_eq(user_text, str(p)):
                score = 100
            elif pp in q:
                score = 60
            else:
                words = [w for w in pp.split() if len(w) > 2]
                if words:
                    score = sum(1 for w in words if w in q) * 10
            if score > best_score:
                best_score = score
                best_answer = item.get("answer", "")
    return best_answer if best_score >= 20 else ""

def physical_is_short_followup(text: str) -> bool:
    q = normalize_text(text)
    if not q:
        return False
    short_tokens = {"soni", "nechta", "qancha", "narxi", "vazni", "kg", "dona", "ha", "yoq", "yo'q", "bor"}
    if q in short_tokens:
        return True
    if len(q.split()) <= 3:
        if any(x in q for x in ["dona", "kg", "gram", "litr", "sum", "dollar"]):
            return True
...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        if not ready_answer:
            for k, v in PHYSICAL_FAQ.get(lang, {}).items():
                if button_eq(text, k):
                    ...
                    break
        if not ready_answer:
            ready_answer = physical_ready_answer_from_big_faq(text, lang)

        if ready_answer:
            physical_clear_state(c)
            topic_guess = physical_pro_guess_topic(text)
            physical_set_pro_topic(c, topic_guess, text)
            physical_mem_set(c, text, topic_guess)
            c["mode"] = "physical_chat"
            await message.answer(
                physical_answer(text, lang) + "\n\n" + t(lang, "physical_free_specialist"),
                reply_markup=physical_ai_kb(lang)
        ...
        if physical_is_short_followup(text):
            mem = physical_mem_get(c)
            if mem.get("text"):
                merged_text = (mem.get("text", "") + " " + text).strip()
                physical_clear_state(c)
                topic_guess = physical_pro_guess_topic(merged_text)
                physical_set_pro_topic(c, topic_guess, merged_text)
                physical_mem_set(c, merged_text, topic_guess)
                c["mode"] = "physical_chat"
                await message.answer(
                    physical_answer(merged_text, lang) + "\n\n" + t(lang, "physical_free_specialist"),
                    reply_markup=physical_ai_kb(lang)
                )
                return
        ...
        if physical_needs_ai_followup(text, c, lang):
            ...
            if ai_text:
                c["mode"] = "physical_chat"
                physical_mem_set(c, text, physical_pro_guess_topic(text))
