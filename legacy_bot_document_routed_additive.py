...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        # NEW: first try document-routed base answer, then big FAQ
        if not ready_answer:
            ready_answer = physical_base_from_documents(text, lang)
        if not ready_answer:
            best = physical_best_faq_item(text, lang)
            if best and best.get("answer"):
                ready_answer = best.get("answer")
...
# ============================================================
# ADDITIVE PATCH: document-routed physical answers
# ============================================================
def _doc_router_load() -> Dict[str, Any]:
    path = os.path.join(BASE_DIR, "physical_faq_document_router.json")
    try:
        ...
        logging.exception("Failed to load physical_faq_document_router.json")
    return {
        "general_rules": {
            "import_limits_usd": {"air": 1000, "rail_river": 500, "car_foot": 300, "courier": 200, "post": 100},
            "duty_rule": {
                "ru": "При превышении лимита применяется единый таможенный платеж: 30% от суммы превышения, но не менее 3 долларов США за каждый килограмм.",
                "uz": "Limit oshsa, yagona bojxona to'lovi qo'llanadi: oshgan summaning 30 foizi, lekin har bir kilogramm uchun kamida 3 AQSh dollaridan kam emas."
            }
        },
        "knowledge": {
            "electronics": {
                "items": {
                    "tv": {"base_answer_ru": "Телевизор можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Televizor 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "phone": {"base_answer_ru": "Телефон можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Telefon 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "laptop": {"base_answer_ru": "Ноутбук можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Noutbukni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "tablet": {"base_answer_ru": "Планшет можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Planshetni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "printer": {"base_answer_ru": "Принтер или МФУ можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Printer yoki ko'p funksiyali ofis qurilmasini 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "computer": {"base_answer_ru": "Компьютерную технику можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Kompyuter texnikasini 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "fridge": {"base_answer_ru": "Холодильник можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Sovutgichni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "freezer": {"base_answer_ru": "Морозильник можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Muzlatgichni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "ac": {"base_answer_ru": "Кондиционер можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Konditsionerni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "washing_machine": {"base_answer_ru": "Стиральную машину можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Kir yuvish mashinasini 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "vacuum": {"base_answer_ru": "Пылесос можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Chang yutgichni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "gas_stove": {"base_answer_ru": "Газовую плиту можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Gaz plitasini 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "electric_stove": {"base_answer_ru": "Электроплиту можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Elektr plitani 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "microwave": {"base_answer_ru": "Микроволновую печь можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Mikroto'lqinli pechni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "oven": {"base_answer_ru": "Духовку можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Duxovkali pechni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "meat_grinder": {"base_answer_ru": "Электрическую мясорубку можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Elektr go'sht qiymalagichni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "iron": {"base_answer_ru": "Утюг можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Dazmolni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "hairdryer": {"base_answer_ru": "Фен можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Fenni 1 dona, 6 oyda 1 marta olib kirish mumkin."},
                    "kitchen_machine": {"base_answer_ru": "Кухонный комбайн можно ввозить 1 штуку в 6 месяцев.", "base_answer_uz": "Oshxona kombaynini 1 dona, 6 oyda 1 marta olib kirish mumkin."}
                },
                "answer_template_ru": "{item_answer} По сумме лимит зависит от поста: аэропорт — до 1000$, авто/пешком — до 300$, ж/д и речные пункты — до 500$. Если стоимость выше лимита, на превышение может начисляться единый таможенный платеж.",
                "answer_template_uz": "{item_answer} Qiymat limiti postga bog'liq: aeroport — 1000$, avto/piyoda — 300$, temir yo'l va daryo punktlari — 500$. Narx limitdan oshsa, oshgan qismiga bojxona to'lovi chiqishi mumkin."
            },
            "medicine": {
                "basic_limit_ru": "Для личного пользования обычно допускается до 10 видов лекарств и не более 5 упаковок каждого.",
                "basic_limit_uz": "Shaxsiy foydalanish uchun odatda 10 xil dori va har biridan 5 qadoqqacha ruxsat etiladi.",
                "forbidden_ru": "Наркотические и психотропные препараты требуют отдельного порядка и документов.",
                "forbidden_uz": "Narkotik va psixotrop dorilar alohida tartib va hujjatlarni talab qiladi."
            },
            "currency": {
                "import_ru": "Ввоз наличной валюты не ограничен по сумме.",
                "import_uz": "Naqd valyutani olib kirish summa bo'yicha cheklanmagan.",
                "export_ru": "Вывоз наличной валюты разрешён в пределах эквивалента 100 000 000 сумов.",
                "export_uz": "Naqd valyutani olib chiqish 100 000 000 so'm ekvivalenti doirasida ruxsat etiladi."
            }
        }
    }

PHYSICAL_DOC_ROUTER = _doc_router_load()
...
def _doc_detect_transport(text: str) -> str:
    q = _doc_norm(text)
    if any(x in q for x in ["аэропорт", "aeroport", "airport", "самолет", "samolyot", "улетаю", "uchyapman", "uchaman"]):
        return "air"
    if any(x in q for x in ["машина", "mashina", "avto", "авто", "на машине"]):
        return "car"
    if any(x in q for x in ["пешком", "piyoda", "пешеход", "chegara"]):
        return "foot"
...
def _doc_detect_electronics_item(text: str) -> str:
    q = _doc_norm(text)
    mapping = {
        "tv": ["телевизор", "televizor", "tv", "смарт тв", "smart tv"],
        "phone": ["телефон", "telefon", "iphone", "айфон", "смартфон", "smartfon"],
        "computer": ["компьютер", "kompyuter", "компьютерная техника", "kompyuter texnikasi"],
        "printer": ["принтер", "printer", "мфу", "ko'p funksiyali"],
        "tablet": ["планшет", "planshet"],
        "laptop": ["ноутбук", "noutbuk", "laptop"],
        "fridge": ["холодильник", "sovutgich"],
        "freezer": ["морозильник", "muzlatgich"],
        "ac": ["кондиционер", "konditsioner"],
        "washing_machine": ["стиральная машина", "kir yuvish mashinasi"],
        "vacuum": ["пылесос", "chang yutgich"],
        "gas_stove": ["газовая плита", "gaz plitasi"],
        "electric_stove": ["электроплита", "elektr plita"],
        "microwave": ["микроволнов", "mikroto'lqinli pech"],
        "oven": ["духов", "duxovkali pech"],
        "meat_grinder": ["мясоруб", "qiymalagich"],
        "iron": ["утюг", "dazmol"],
        "hairdryer": ["фен", "fen"],
        "kitchen_machine": ["кухонный комбайн", "oshxona kombayni"]
    }
...
def _doc_parse_amount_usd(text: str) -> float:
    q = _doc_norm(text)
...
def physical_base_from_documents(text: str, lang: str) -> str:
    q = _doc_norm(text)
    transport = _doc_detect_transport(text)
    item_key = _doc_detect_electronics_item(text)

    # electronics from VMQ 463
    if item_key:
        item = PHYSICAL_DOC_ROUTER.get("knowledge", {}).get("electronics", {}).get("items", {}).get(item_key, {})
        if item:
            item_answer = item.get("base_answer_ru" if lang == "ru" else "base_answer_uz", "")
            template = PHYSICAL_DOC_ROUTER.get("knowledge", {}).get("electronics", {}).get("answer_template_ru" if lang == "ru" else "answer_template_uz", "{item_answer}")
            answer = template.replace("{item_answer}", item_answer)

            amount = _doc_parse_amount_usd(text)
            limits = PHYSICAL_DOC_ROUTER.get("general_rules", {}).get("import_limits_usd", {})
            if amount and transport and transport in limits:
                limit = limits.get(transport, 0)
                if amount > limit:
                    excess = round(amount - limit, 2)
                    duty_text = PHYSICAL_DOC_ROUTER.get("general_rules", {}).get("duty_rule", {}).get(lang, "")
                    if lang == "ru":
                        answer += f"\n\nВ вашем случае лимит по посту — {limit}$. Стоимость товара: {amount}$. Превышение: {excess}$.\n{duty_text}"
                    else:
                        answer += f"\n\nSizning holatingizda post bo'yicha limit — {limit}$. Tovar narxi: {amount}$. Oshgan qismi: {excess}$.\n{duty_text}"
            return answer

    # medicine short base
    if any(x in q for x in ["лекар", "таблет", "цитрамон", "парацетамол", "ибупрофен", "анальгин", "dori", "tabletka", "sitramon", "paratsetamol", "ibuprofen", "analgin"]):
        med = PHYSICAL_DOC_ROUTER.get("knowledge", {}).get("medicine", {})
        if any(x in q for x in ["наркот", "психотроп", "narkotik", "psixotrop"]):
            return med.get("forbidden_ru" if lang == "ru" else "forbidden_uz", "")
        return med.get("basic_limit_ru" if lang == "ru" else "basic_limit_uz", "")

    # currency short base
    if any(x in q for x in ["валют", "доллар", "евро", "налич", "деньги", "valyuta", "dollar", "yevro", "naqd", "pul"]):
        cur = PHYSICAL_DOC_ROUTER.get("knowledge", {}).get("currency", {})
        if any(x in q for x in ["вывоз", "вывез", "улетаю", "с собой", "olib chiq", "olib ket", "uchyapman", "uchaman"]):
            return cur.get("export_ru" if lang == "ru" else "export_uz", "")
        return cur.get("import_ru" if lang == "ru" else "import_uz", "")

    return ""

# Override physical pattern answer additively:
_physical_pro_pattern_answer_old = physical_pro_pattern_answer
def physical_pro_pattern_answer(text: str, lang: str) -> str:
    base = physical_base_from_documents(text, lang)
    if base:
        return base
    return _physical_pro_pattern_answer_old(text, lang)

_physical_answer_old = physical_answer
def physical_answer(text: str, lang: str) -> str:
    base = physical_base_from_documents(text, lang)
    if base:
        return base
    return _physical_answer_old(text, lang)
