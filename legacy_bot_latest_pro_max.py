    ...
    try:
        ...
        for lang in ("ru", "uz"):
            ...
            if isinstance(block, dict):
                ...
                global_rules = block.get("global_rules", {})
                if isinstance(faq_items, list):
                    prepared[lang]["faq_items"] = faq_items
                if isinstance(global_rules, dict):
                    prepared[lang]["global_rules"] = global_rules
        return prepared
    except Exception as e:
        print(f"Failed to load physical_faq.json: {e}")
        return empty
...
# ============================================================
# FINAL PRO MAX PHYSICAL OVERRIDES
# Built on user's latest legacy_bot.py, other sections untouched.
# ============================================================

_PM_APOS_MAP = str.maketrans({
    "‘": "'",
    "’": "'",
    "ʼ": "'",
    "ʻ": "'",
...
def pm_normalize(text: str) -> str:
    s = (text or "").lower().translate(_PM_APOS_MAP).replace("ё", "е")
...
def physical_get_pro_topic(c: Dict[str, Any]) -> str:
    return c.get("physical_pro_state", {}).get("topic", "")

def physical_clear_pro_state(c: Dict[str, Any]) -> None:
    c["physical_pro_state"] = {}

def pm_detect_direction(text: str) -> str:
    q = pm_normalize(text)
    if any(x in q for x in ["вывез", "вывоз", "улетаю", "лечу", "беру с собой", "с собой", "olib chiq", "olib ket", "uchyapman", "uchaman"]):
        return "export"
    if any(x in q for x in ["ввез", "ввоз", "привез", "завез", "olib kir", "olib kel", "возвращаюсь", "kiryapman"]):
        return "import"
    return ""

def pm_detect_transport(text: str) -> str:
    q = pm_normalize(text)
    if any(x in q for x in ["аэропорт", "airport", "aeroport", "самолет", "самолетом", "рейс", "улетаю", "лечу", "uchyapman", "uchaman"]):
        return "air"
    if any(x in q for x in ["машина", "авто", "на машине", "mashina", "avto"]):
        return "car"
    if any(x in q for x in ["пешком", "пеший", "piyoda"]):
        return "foot"
    if any(x in q for x in ["поезд", "жд", "temir", "poyezd"]):
        return "rail"
    if any(x in q for x in ["почта", "pochta", "посылка", "posilka"]):
        return "mail"
    if any(x in q for x in ["курьер", "kuryer", "доставка"]):
        return "courier"
    return ""

def pm_is_migration_question(text: str) -> bool:
    q = pm_normalize(text)
    migration_words = [
        "сколько дней", "срок пребывания", "регистрация иностранца", "виза",
        "как долго можно", "гражданин россии", "россиянин", "миграц",
        "fuqarosi", "necha kun", "nechchi kun", "bo'lishi mumkin", "bo'lish",
        "qancha muddat", "migrats", "ro'yxatdan o'tish", "royxatdan otish", "kirib yurish muddati"
    ]
    goods_words = [
        "телефон", "айфон", "лекар", "dori", "таблет", "валют", "valyuta",
        "золото", "tilla", "сигар", "alkog", "алког", "хлеб", "non", "ovqat",
        "одежд", "kiyim", "мясо", "gosht", "еда", "товар", "tovar"
    ]
    return any(w in q for w in migration_words) and not any(w in q for w in goods_words)

def pm_migration_redirect(lang: str) -> str:
    return (
        "Это вопрос не про таможенные товары, а про миграцию, срок пребывания или регистрацию иностранца. По таким вопросам лучше смотреть официальные миграционные правила или обращаться в профильный орган."
        if lang == "ru" else
        "Bu savol bojxona tovarlari haqida emas, balki migratsiya, bo‘lish muddati yoki ro‘yxatdan o‘tish haqida. Bunday savollar bo‘yicha rasmiy migratsiya qoidalarini tekshirish yoki vakolatli organga murojaat qilish kerak."
    )

def physical_pro_guess_topic(text: str) -> str:
    q = pm_normalize(text)
    topic_keywords = {
        "medicine": ["лекар", "dori", "tablet", "таблет", "цитрамон", "sitramon", "paratsetamol", "парацетамол", "ibuprofen", "ибупрофен", "анальгин", "retsept", "рецепт", "antibiotik", "антибиот"],
        "phones": ["телефон", "айфон", "iphone", "смартфон", "telefon", "smartfon", "imei", "phone"],
        "tv": ["телевизор", "televizor", "tv", "smart tv", "смарт тв"],
        "notebook": ["ноутбук", "noutbuk", "laptop", "макбук", "macbook", "planshet", "планшет", "tablet pc"],
        "currency": ["валют", "valyuta", "доллар", "usd", "евро", "eur", "sum", "сум", "налич", "naqd", "pul"],
        "tobacco_alcohol": ["сигар", "табак", "tamaki", "alkogol", "алког", "пиво", "vino", "коньяк", "vodka", "вино", "спирт", "sigaret"],
        "perfume": ["парфюм", "парфюмер", "духи", "atir", "parfyum"],
        "jewelry": ["золото", "tilla", "цепоч", "kulon", "кулон", "кольц", "uzuk", "серьг", "zirak", "braslet", "браслет", "ювелир", "taqinchoq"],
        "food": ["non", "хлеб", "еда", "ovqat", "oziq", "конфет", "печень", "сыр", "мясо", "gosht", "go'sht", "яблок", "olma", "meva", "фрукт", "колбас", "домашн", "плов", "печенье", "печен", "хлеба", "булка"],
        "clothes": ["одежд", "kiyim", "eski kiyim", "вещ", "обув", "куртк", "чемодан одежды", "старая одежда", "eski", "oyoq kiyim", "кроссовк", "ботинк"],
        "pyro": ["пиротех", "салют", "petard", "pirotex", "feyerverk", "петарда"],
        "drone": ["дрон", "kvadrokopter", "квадрокоптер", "drone", "bepilot"],
        "auto": ["авто", "машин", "автомоб", "mashina", "avto", "tesla", "byd", "гибрид", "gibrid", "электромоб"],
        "forbidden": ["что нельзя", "что запрещено", "zapret", "taqiql", "оруж", "qurol", "боеприпас", "наркот", "psixotrop", "прекурсор"],
        "boundary": ["на продажу", "для продажи", "коммерц", "магазин", "biznes", "sotish uchun", "partiya", "tn ved", "тн вэд", "broker"],
    }
    best_topic = ""
    best_score = 0
    ...
    if best_topic:
        return best_topic
    if any(x in q for x in ["улетаю", "лечу", "беру с собой", "с собой", "olib chiq", "olib ket", "uchyapman", "uchaman"]):
        return "export"
    if any(x in q for x in ["ввез", "ввоз", "привез", "завез", "olib kir", "olib kel"]):
        return "import"
    return ""

def pm_weighted_faq_search(text: str, lang: str) -> Dict[str, Any]:
    q = pm_normalize(text)
    topic = physical_pro_guess_topic(text)
    direction = pm_detect_direction(text)
    transport = pm_detect_transport(text)

    best = None
    best_score = -1
    items = PHYSICAL_FAQ_PRO.get(lang, {}).get("faq_items", [])
    for item in items:
        score = 0
        item_topic = item.get("topic", "")
        patterns = item.get("patterns", []) or []
        for p in patterns:
            pn = pm_normalize(str(p))
            if not pn:
                continue
            if pn == q:
                score += 120
            ...
            if words and all(w in q for w in words[:min(4, len(words))]):
                score += 18

        if topic and item_topic == topic:
            score += 35
        if topic == "food" and item_topic in {"food", "context"}:
            score += 18
        if topic == "clothes" and item_topic in {"clothes", "context"}:
            score += 18
        if topic == "jewelry" and item_topic == "jewelry":
            score += 20
        if topic == "phones" and item_topic == "phones":
            score += 20
        if topic == "medicine" and item_topic == "medicine":
            score += 20
        if topic == "currency" and item_topic == "currency":
            score += 20
        if topic == "forbidden" and item_topic in {"forbidden", "pyro", "drone"}:
            score += 18

        if direction == "export" and any(x in q for x in ["улетаю", "лечу", "беру с собой", "olib chiq", "olib ket"]):
            if item_topic in {"jewelry", "currency", "food", "clothes", "forbidden", "context", "tobacco_alcohol", "perfume"}:
                score += 12
        if direction == "import" and any(x in q for x in ["ввез", "привез", "завез", "olib kir", "olib kel"]):
            if item_topic in {"phones", "medicine", "tobacco_alcohol", "perfume", "jewelry", "food", "clothes", "limits"}:
                score += 12

        if transport == "air" and any(x in q for x in ["улетаю", "лечу", "aeroport", "аэропорт", "uchyapman", "uchaman"]):
            score += 8
...
def physical_pro_pattern_answer(text: str, lang: str) -> str:
    if pm_is_migration_question(text):
        return pm_migration_redirect(lang)

    found = pm_weighted_faq_search(text, lang)
    if found and found.get("_score", 0) >= 24:
        return found.get("answer", "")

    topic = physical_pro_guess_topic(text)
    if topic == "food":
        return (
            "Если речь об обычных продуктах питания для личного пользования, важны вид продукта, количество и в какую страну вы его везёте. По обычному хлебу или еде в небольшом количестве вопрос обычно проще, но окончательно лучше оценивать по стране назначения и объёму."
            if lang == "ru" else
            "Agar gap oddiy oziq-ovqat mahsulotlari haqida bo‘lsa, mahsulot turi, miqdori va qaysi davlatga olib ketayotganingiz muhim. Oddiy non yoki ovqatni oz miqdorda olib chiqish masalasi odatda osonroq bo‘ladi, lekin yakuniy baho mamlakat va hajmga bog‘liq."
        )
    if topic == "clothes":
        return (
            "Личные ношеные вещи для себя обычно рассматриваются как личное пользование, если это не выглядит как товарная партия. Если одежды слишком много, таможня может усомниться в характере товара."
            if lang == "ru" else
            "O‘zingizning eski kiyimlaringiz odatda shaxsiy foydalanish sifatida baholanadi, agar bu tovar partiyasiga o‘xshamasa. Kiyim juda ko‘p bo‘lsa, bojxona savol berishi mumkin."
        )
    return ""
...
def physical_pro_global_hint(text: str, lang: str) -> str:
    ...
    if direction == "export" and transport == "air" and topic == "":
        return (
            "Если вы улетаете, значит речь идёт о выезде через аэропорт. Напишите, пожалуйста, что именно хотите взять с собой: валюту, золото, лекарства, телефон, еду, одежду или другой товар."
            if lang == "ru" else
            "Agar uchayotgan bo‘lsangiz, demak gap aeroport orqali olib chiqish haqida ketmoqda. O‘zingiz bilan nima olib ketmoqchi ekaningizni yozing: valyuta, tilla, dori, telefon, ovqat, kiyim yoki boshqa tovar."
        )

    if topic == "forbidden" or any(x in q for x in ["что нельзя", "что запрещено", "zapret", "taqiql"]):
        return (
            "Нужно смотреть по конкретному товару: часть товаров запрещена, часть требует разрешения, а обычные личные вещи оцениваются отдельно. Напишите, пожалуйста, какой именно товар вас интересует."
            if lang == "ru" else
            "Aniq tovarni ko‘rish kerak: ayrim tovarlar taqiqlangan, ayrimlari ruxsat bilan, oddiy shaxsiy buyumlar esa alohida baholanadi. Qaysi tovar haqida so‘rayotganingizni yozing."
        )

    if topic == "currency":
        item = rules.get("currency_rules", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    if topic == "medicine":
        found = pm_weighted_faq_search("сколько лекарств можно" if lang == "ru" else "dori qancha olib kirish mumkin", lang)
        if found:
            return found.get("answer", "")
    if topic == "auto":
        found = pm_weighted_faq_search("машина" if lang == "ru" else "mashina", lang)
        if found:
            return found.get("answer", "")
    if topic in {"import", "tv", "phones", "notebook"} or (direction == "import" and transport):
        item = rules.get("import_limits", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    if topic == "tobacco_alcohol":
        item = rules.get("tobacco_alcohol_limits", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    if topic == "perfume":
        item = rules.get("perfume_limits", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    if direction == "export":
        item = rules.get("export_goods_limit", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    if topic == "boundary":
        item = rules.get("commercial_redirect", {})
        return item.get("answer", "") if isinstance(item, dict) else ""
    return ""
...
def physical_answer(text: str, lang: str) -> str:
    ...
    if topic == "food":
        return (
            "Если речь о продуктах питания для личного пользования, напишите, пожалуйста: какой именно продукт, сколько единиц и в какую страну вы его везёте."
            if lang == "ru" else
            "Agar gap oziq-ovqat haqida bo‘lsa, iltimos yozing: aynan qaysi mahsulot, nechta dona va qaysi davlatga olib ketmoqchisiz."
        )
    if topic == "clothes":
        return (
            "Если речь об одежде и вещах для себя, важны количество и признаки личного пользования. Напишите, сколько примерно вещей и это личные ношеные вещи или новые товары."
            if lang == "ru" else
            "Agar gap kiyim va shaxsiy buyumlar haqida bo‘lsa, miqdor va shaxsiy foydalanish alomatlari muhim. Taxminan qancha narsa va ular eski shaxsiy buyumlarmi yoki yangi tovarlarmi — yozing."
        )
    return physical_answer_legacy(text, lang)
...
def physical_needs_ai_followup(text: str, c: Dict[str, Any], lang: str) -> bool:
    ...
    if topic in {"medicine", "tv", "phones", "notebook", "currency", "auto", "food", "clothes", "jewelry", "import"}:
        if any(ch.isdigit() for ch in text):
            return True
        if 1 <= len(q.split()) <= 8 and not any(x in q for x in ["можно", "mumkin", "сколько", "qancha", "какие", "qaysi", "nima", "какой"]):
            return True
    return False

def build_physical_ai_prompt(user_text: str, c: Dict[str, Any], lang: str) -> str:
    topic = physical_get_pro_topic(c) or physical_pro_guess_topic(user_text)
    direction = pm_detect_direction(user_text)
    transport = pm_detect_transport(user_text)
    source_text = c.get("physical_pro_state", {}).get("source_text", "")
    local = []
    patt = physical_pro_pattern_answer(user_text, lang)
    if patt:
        local.append(patt)
    ...
    ctx = f"\n\nTopic: {topic or '-'}\nDirection: {direction or '-'}\nTransport: {transport or '-'}"
    if source_text:
        ctx += f"\nPrevious user context: {source_text[:250]}"
    if local:
        ctx += "\n\nLocal context:\n- " + "\n- ".join(local)
    return base + ctx
...
def looks_like_physical_customs_question(text: str) -> bool:
    ...
    markers = [
        ...
        "для себя", "личного пользования", "shaxsiy foydalanish",
        "аэропорт", "airport", "aeroport", "post", "пост", "границ", "chegara",
        "вывезти", "ввезти", "olib kir", "olib chiq", "из дубая", "dubay",
        "из казахстана", "qozog'iston", "rossiya", "кыргызстан",
        "non", "хлеб", "еда", "ovqat", "oziq", "одежд", "kiyim", "eski kiyim", "вещи",
        "золото", "tilla", "цепочка", "kulon", "сигар", "alkogol", "дрон", "pirotex"
    ]
    return any(m in q for m in markers)
