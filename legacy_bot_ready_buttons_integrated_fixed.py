...
def map_ready_question(lang: str, user_text: str) -> str:
    ready_keys = ["physical_q1", "physical_q2", "physical_q3", "physical_q4", "physical_q5", "physical_q6"]
    for key in ready_keys:
        if button_eq(user_text, t(lang, key)):
            return t(lang, key)
    return ""
...
async def router(message: types.Message):
    ...
    if role == "physical":
        ...
        for _ready_key in ["physical_q1", "physical_q2", "physical_q3", "physical_q4", "physical_q5", "physical_q6"]:
            if button_eq(text, t(lang, _ready_key)):
                ...
                clear_physical_context(c)
                c["mode"] = "physical_chat"
                ready_text = map_ready_question(lang, text)
                pro_answer = find_physical_answer_v2(uid, ready_text)
                if pro_answer:
    ...
    if c["mode"] == "physical_chat":
        ...
        await message.answer(t(lang, "nothing_found"), reply_markup=physical_ai_kb(lang))
        return
