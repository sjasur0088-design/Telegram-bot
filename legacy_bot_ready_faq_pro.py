...
def load_physical_faq_pro() -> Dict[str, Dict[str, Any]]:
    path = os.path.join(BASE_DIR, "physical_faq_new_pro.json")
    empty = {"ru": {"faq_items": [], "global_rules": {}}, "uz": {"faq_items": [], "global_rules": {}}}

    try:
        if not os.path.exists(path):
            print(f"physical_faq_new_pro.json not found: {path}")
            return empty
        ...
        print("Loaded physical_faq_new_pro.json successfully")
        return prepared
    except Exception as e:
        print(f"Failed to load physical_faq_new_pro.json: {e}")
        return empty
