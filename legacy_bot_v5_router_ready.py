...
async def router(message: types.Message):
    ...
    if c["mode"] == "physical_ready":
        ...
        # 1) First try document-routed short answer
        if not ready_answer:
            ready_answer = physical_base_from_documents_v5(text, lang)

        # 2) Then try the big physical_faq.json
        if not ready_answer:
            best = physical_best_faq_item(text, lang)
...
# This patch only upgrades physical-person logic.
...
def _v5_norm(s: str) -> str:
    s = (s or "").lower()
    s = s.replace("ё", "е").replace("ʻ", "'").replace("ʼ", "'").replace("`", "'").replace("’", "'").replace("‘", "'")
...
def _v5_match_named_item(text: str, block: Dict[str, Any]) -> str:
    ...
    for item_key, item in block.get("items", {}).items():
        names_ru = item.get("names_ru", []) if isinstance(item, dict) else []
        names_uz = item.get("names_uz", []) if isinstance(item, dict) else []
        all_names = list(names_ru) + list(names_uz)
        if any(_v5_norm(n) in q for n in all_names):
            return item_key
    return ""
...
def physical_base_from_documents_v5(text: str, lang: str) -> str:
    ...
    if any(x in q for x in ["лекар", "лекарства", "таблет", "dori", "dorilar", "tabletka"]):
        # common drugs first
        for drug_key, drug in medicine.get("common_drugs", {}).items():
    ...
    if any(x in q for x in ["валют", "доллар", "евро", "налич", "деньги", "valyuta", "dollar", "yevro", "evro", "naqd", "pul"]):
        ...
        # if amount-like large case is implied, append declaration note
...
# additive override: first document router, then existing logic
