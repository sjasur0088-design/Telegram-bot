...
class PhysicalRouter:
    ...
    def normalize_text(self, text: str) -> str:
        ...
        replacements = {
            "ё": "е", "телик": "телевизор", "айфон": "телефон", "смарт тв": "smart tv", "смарт-тв": "smart tv",
            "ж/д": "жд", "самолётом": "самолетом", "самолёт": "самолет", "пограничный пункт": "граница",
            "пешеходный пункт": "пешком", "через границу": "граница", "usd": "доллар", "eur": "евро",
            "у.е.": "доллар", "уе": "доллар", "kg": "кг", "килограмма": "кг", "килограммов": "кг",
            "килограмм": "кг", "долларов": "доллар", "доллара": "доллар", "сумов": "сум", "сума": "сум",
        }
    ...
    def detect_category(self, text_norm: str, lang: str) -> Dict[str, Any]:
        scores = {name: self._score_category(text_norm, name, lang) for name in self.categories.keys()}
        ...
        return {"category": best_category, "score": best_score, "ranked": ranked}
    ...
    def _extract_price(self, text_norm: str):
        explicit_patterns = [r"(\d+(?:[.,]\d+)?)\s*(доллар|евро|сум)", r"(\d+(?:[.,]\d+)?)\s*(\$)"]
        ...
        try:
            ...
            return None
        if "евро" in text_norm:
            currency = "евро"
        elif "сум" in text_norm:
            currency = "сум"
        else:
            currency = "доллар"
        return {"value": value, "currency": currency}

    def _extract_weight(self, text_norm: str):
    ...
    def _extract_quantity(self, text_norm: str):
        patterns=[r"(\d+)\s*(штук|штуки|штука|дона|упаковок|упаковки|упаковка|телефона|телефон|товара)",r"везу\s+(\d+)",r"хочу привезти\s+(\d+)"]
    ...
    def detect_intent(self, text_norm, facts, lang):
        if facts.get("price"):
            return "calculation"
        if facts.get("crossing_type") or facts.get("weight_kg"):
            return "calculation"
        quantity = facts.get("quantity")
        if quantity and quantity > 1:
            return "commercial_risk"
        for marker in self.intent_markers.get("over_limit", {}).get(lang, []):
            if marker in text_norm:
                return "over_limit"
        for marker in self.intent_markers.get("quantity_limit", {}).get(lang, []):
            if marker in text_norm:
                return "quantity_limit"
        for marker in self.intent_markers.get("basic_permission", {}).get(lang, []):
            if marker in text_norm:
    ...
    def resolve_followup(self, text_norm: str, context: Optional[Dict[str, Any]]):
        if not context:
            return {}
        followup_hits = 0
        ...
        if followup_hits > 0 or short_message:
            return {"is_followup": True, "use_context": True, "context_category": context.get("last_category"), "context_item": context.get("last_item"), "context_intent": context.get("last_intent")}
    ...
    def route(self, text: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if followup.get("use_context") and followup.get("context_category") and category_info["score"] < 5:
        ...
        if followup.get("use_context") and intent == "item_rule":
            if facts.get("price") or facts.get("crossing_type") or facts.get("weight_kg"):
                intent = "calculation"
        return {"text_norm": text_norm, "lang": lang, "category": category, "item": item, "intent": intent, "facts": facts, "category_score": category_info["score"], "category_ranked": category_info["ranked"], "followup": followup}
